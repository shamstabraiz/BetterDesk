...
fn warn_self_signed_once() {
    ...
    if !WARNED.swap(true, Ordering::SeqCst) {
        warn!(
            "TLS certificate validation is DISABLED for codenextremote API calls. \
...
async fn try_resolve_api_endpoint(address: &str) -> Result<ResolvedApiEndpoint> {
    ...
    for port in candidate_ports(input_port) {
        for scheme in candidate_schemes(explicit_scheme.as_deref()) {
            ...
            {
                info!(
                    "Resolved codenextremote Go API endpoint: {}://{}:{}",
                    scheme, host, port
                );
    ...
    Err(anyhow!(
        "Could not find codenextremote Go API for {}. Use the server host or select a discovered LAN server.",
        address.trim()
    ))
}
...
pub async fn normalize_server_origin_best_effort(config: &mut AgentConfig) {
    ...
    info!(
        "Normalized codenextremote server origin for API/CDAP: {} -> {}",
        current,
        origin
    );
...
/// Step 2: Verify the server speaks codenextremote protocol.
async fn check_protocol(address: &str) -> Result<String> {
    ...
    let body: serde_json::Value = resp.json().await.map_err(|_| {
        anyhow!("Server response is not valid JSON — not a codenextremote server")
    })?;

    // codenextremote Go server /api/server/stats returns {"peers_count": N, ...}
    if body.get("peers_count").is_some() || body.get("version").is_some() {
        Ok("codenextremote protocol confirmed".to_string())
    } else {
        Err(anyhow!("Server does not appear to be a codenextremote server"))
    }
}
...
async fn check_registration_open(address: &str) -> Result<String> {
    ...
    match resp {
        ...
        Ok(r) => {
            // Some codenextremote deployments do not expose a dedicated probe here.
...
/// Register this device with the codenextremote server.
...
pub async fn register(config: &mut AgentConfig) -> Result<String> {
    ...
    // across the codenextremote/RustDesk surfaces. The full machine UUID still
    ...
    loop {
        ...
        match tokio::time::timeout(remaining, socket.recv_from(&mut buf)).await {
            Ok(Ok((len, addr))) => {
                ...
                if response.msg_type != "codenextremote-announce" {
                    continue;
                }
