...
pub struct AgentConfig {
    ...
    /// "https://codenextremote.example.com:5443" so follow-up API/CDAP calls can
    /// preserve the correct transport scheme.
    pub server_address: String,

    /// API key used to authenticate the sidecar Go agent with the CDAP gateway.
    /// Obtained from the codenextremote admin panel → API Keys.
    #[serde(default)]
    pub api_key: String,
...
impl AgentConfig {
    ...
    fn config_path() -> PathBuf {
        let dir = directories::ProjectDirs::from("com", "codenextremote", "agent")
    ...
    pub fn to_cdap_config(&self) -> crate::cdap_client::CdapConfig {
        let data_dir = directories::ProjectDirs::from("com", "codenextremote", "agent")
    ...
    pub fn store_token_secure(&self) -> Result<()> {
        ...
        let entry = keyring::Entry::new("codenextremote-agent", &self.device_id)?;
    ...
    pub fn load_token_secure(device_id: &str) -> Option<String> {
        keyring::Entry::new("codenextremote-agent", device_id)
            .ok()
    ...
    pub fn clear_token_secure(device_id: &str) {
        if let Ok(entry) = keyring::Entry::new("codenextremote-agent", device_id) {
