//! Sidecar manager — runs `codenextremote-agent` (Go binary) as a managed child
...
//!    dir and spawns `codenextremote-agent -config <path>`.
...
//!      (`<exe-dir>/codenextremote-agent[.exe]`).
//!   3. App data dir (`<data>/codenextremote-agent[.exe]`).
...
/// Fields match `codenextremote-agent/agent/config.go`.
...
impl SidecarManager {
    ...
    fn spawn_process(&self, binary: &PathBuf, config_path: &PathBuf) -> Result<()> {
        let mut child = Command::new(binary)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .with_context(|| format!("spawn codenextremote-agent from {}", binary.display()))?;
        ...
        *self.inner.child_stdin.lock().unwrap() = child_stdin;
        ...
        info!("[sidecar] Spawned codenextremote-agent (pid={})", pid);
        Ok(())
    }
...
/// Find the `codenextremote-agent` binary.
...
fn find_binary(data_dir: &PathBuf) -> Result<PathBuf> {
    ...
    Err(anyhow!(
        "codenextremote-agent binary not found. Searched: \
         $BETTERDESK_AGENT_BIN, exe dir, {}, PATH. \
         Download from https://github.com/shamstabraiz/codenextremote/releases \
...
/// Write the Go-format JSON config file consumed by codenextremote-agent.
