...
/// Compile the `codenextremote-agent` Go binary and place it in
/// `src-tauri/binaries/` using the Tauri externalBin naming convention:
/// `codenextremote-agent-<target-triple>[.exe]`.
///
...
fn build_go_sidecar() {
    ...
    // The agent lives two levels up: <repo>/codenextremote-agent/
    let agent_dir = manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .map(|p| p.join("codenextremote-agent"))
        .unwrap_or_default();

    if !agent_dir.exists() {
        println!(
            "cargo:warning=[sidecar] codenextremote-agent not found at {:?} — skipping Go build",
            agent_dir
        );
        return;
    }
    ...
    let bin_name = if goos == "windows" {
        format!("codenextremote-agent-{}.exe", target_triple)
    } else {
        format!("codenextremote-agent-{}", target_triple)
    };
