...
CDAP (Connected Device Automation Protocol) agent for codenextremote. Connects to a codenextremote server's CDAP gateway and provides system monitoring, remote terminal, file browser, clipboard sync, and screenshot capabilities.
...
| `-data-dir` | Data directory | `/var/lib/codenextremote-agent` |
...
# Linux AMD64
GOOS=linux GOARCH=amd64 go build -o codenextremote-agent-linux-amd64 .

# Linux ARM64
GOOS=linux GOARCH=arm64 go build -o codenextremote-agent-linux-arm64 .

# Windows
GOOS=windows GOARCH=amd64 go build -o codenextremote-agent.exe .
```
...
Same as codenextremote project — see repository LICENSE.
