# codenextremote Console - Changelog
...
- **codenextremote.sh / codenextremote.ps1**: Removed `-tls-api -force-https` from Go server args for ALL certificate types. Signal/relay TLS unchanged.
...
- **codenextremote.sh**: Added `$` → `$$` escaping for admin password and PostgreSQL URL before writing to `.service` files.
...
- **codenextremote.sh**: Added `MainThread` to expected process patterns for ports 5000 and 21121.
...
- **Cookie Name Fix** — Logout now correctly clears `codenextremote.sid` (was `connect.sid`)
...
- New menu option **C** in both `codenextremote.sh` and `codenextremote.ps1`
...
- `Install-FlaskConsole` function removed from `codenextremote.ps1`
...
- `codenextremote.sh` — v2.3.0 with Flask removal and SSL wizard
- `codenextremote.ps1` — v2.3.0 with Flask removal and SSL configuration
- `VERSION` — 2.3.0
...
- Compiled from RustDesk Server 1.1.14 with codenextremote enhancements
...
3. Delete `/opt/codenextremoteConsole/admin_credentials.txt`
...
3. Reload services: `systemctl daemon-reload && systemctl restart rustdesksignal codenextremote`
...
- **Compatibility**: Works with existing codenextremote Console database schema
