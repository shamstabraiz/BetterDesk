# Audyt bezpieczeństwa i stabilności codenextremote

**Data:** 2026-04-17
**Zakres:** Pełny audyt projektu shamstabraiz/Rustdesk-FreeConsole (codenextremote)
...
2. **MITM w klientach Tauri** — `danger_accept_invalid_certs(true)` w `codenextremote-mgmt` i `codenextremote-agent-client` pozwala dowolnemu atakującemu na przejęcie rejestracji/sesji operatora; usunąć bezwarunkową akceptację i wprowadzić potwierdzenie użytkownika + pinning fingerprintu.
...
### 3.1 Go Server (`codenextremote-server/`)
...
### 3.3 MGMT Client (`codenextremote-mgmt/`) — Tauri + SolidJS
...
### 3.4 Agent Client (`codenextremote-agent-client/`) — Tauri + SolidJS
...
### 3.5 Natywny Go Agent (`codenextremote-agent/`)
...
`codenextremote.sh`, `codenextremote.ps1`, `codenextremote-docker.sh`, `Dockerfile*`, `docker/entrypoint.sh`, `docker-compose*.yml`.

#### Znaleziska Wysokie (1)

- H1: `codenextremote.sh:1396` — `curl -fsSL … | bash -` (NodeSource) bez weryfikacji SHA256. Best practice: pobrać → `sha256sum -c` → `bash`.
...
- ✅ Non-root user w Dockerfile (`codenextremote:10001`).
- ✅ `COPY --chown=codenextremote:codenextremote` + `tini` + `su-exec`.
...
8. **Dzień 3-4:** Windows-specific keyring fallback do `sodiumoxide`-encrypted pliku w `%APPDATA%\codenextremote\secrets.enc`.
...
12. **Integracja z CDAP gateway** — użyć natywnego `codenextremote-agent` jako library albo przepisać w Rust (terminal przez `portable-pty`, file browser, clipboard przez `arboard`, screenshot przez `scrap`/`screenshots-rs`).
...
14. **Auto-update mechanism** — Tauri updater plugin + signature verification (Ed25519 sign by codenextremote CA).
