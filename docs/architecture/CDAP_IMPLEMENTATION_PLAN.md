...
- **Native codenextremote client** — Desktop agent that combines remote desktop + OS management
...
**Problem**: When a codenextremote native client (CDAP) runs on the same machine as a RustDesk client, or replaces it, the following must be synchronized:
...
**Scenario**: User A has codenextremote native client. User B has RustDesk client. Can User A remote-control User B's machine?
...
The server-side protocol bridge (Phase 6) is complex and optional. The primary migration path is: install codenextremote native client → device appears as both `rustdesk` AND `desktop` types → gradually deprecate RustDesk client on that machine.

### AD-6: Unified Device Identity

When a machine has both RustDesk and codenextremote native clients:
...
The codenextremote native desktop client needs a full 2FA UI:

```
┌──────────────────────────────────────┐
│       codenextremote - Login             │
│                                      │
│  Server: codenextremote.example.com      │
│                                      │
│  Username: [operator1        ]       │
│  Password: [••••••••         ]       │
│                                      │
...
A codenextremote deployment typically has a mix of:
- **Existing RustDesk clients** — already deployed on workstations, using RustDesk protocol on port 21116
- **New codenextremote native clients** (CDAP) — being rolled out, using CDAP on port 21122
...
When a machine runs both RustDesk and codenextremote native clients:
...
### Phase 6: Native codenextremote Desktop Agent (10-15 days)

| # | Task | Effort | Files |
|---|------|--------|-------|
| 6.1 | Agent binary scaffold (Rust or Go) — systemd/service installer | 2d | `codenextremote-agent/` (NEW repo or dir) |
| 6.2 | CDAP client library — auth + heartbeat + manifest | 2d | `codenextremote-agent/cdap/` |
| 6.3 | Screen capture — DXGI (Windows), X11/PipeWire (Linux) | 2d | `codenextremote-agent/capture/` |
| 6.4 | Video encoder — H.264/VP9 hardware-accelerated | 2d | `codenextremote-agent/encoder/` |
| 6.5 | Audio capture — WASAPI (Windows), PulseAudio (Linux) | 1d | `codenextremote-agent/audio/` |
| 6.6 | Input injection — keyboard/mouse (platform-specific) | 1d | `codenextremote-agent/input/` |
| 6.7 | Clipboard monitor — text/image sync | 1d | `codenextremote-agent/clipboard/` |
| 6.8 | File transfer — chunk read/write with resume | 1d | `codenextremote-agent/files/` |
| 6.9 | System widgets — CPU/RAM/disk/services/processes | 1d | `codenextremote-agent/sysinfo/` |
| 6.10 | Login UI — username/password + 2FA dialog | 1d | `codenextremote-agent/ui/` |
| 6.11 | Tray icon + auto-start | 0.5d | `codenextremote-agent/ui/` |
| 6.12 | Update from ALL-IN-ONE scripts (install/update support) | 1d | `codenextremote.sh`, `codenextremote.ps1` |

**Gate**: codenextremote agent installs on Windows/Linux, logs in with 2FA, appears as `desktop` type in panel, supports remote desktop + system management widgets.
...
### Go Server (`codenextremote-server/`)
...
| `services/codenextremoteApi.js` | Modify | 2 | Add CDAP API methods |

### ALL-IN-ONE Scripts

| File | Action | Phase | Description |
|------|--------|-------|-------------|
| `codenextremote.sh` | Modify | 6 | CDAP port in firewall, agent install option |
| `codenextremote.ps1` | Modify | 6 | Same for Windows |
| `codenextremote-docker.sh` | Modify | 1 | Expose port 21122 in docker-compose |
...
```
Step 1: Update codenextremote server (Phase 1-4)
...
Step 3: Deploy codenextremote native agent on select machines (Phase 6)
...
| RustDesk protocol changes break existing | Medium | Low | codenextremote server frozen at RustDesk 1.3.x compat |
...
| RustDesk client cannot be config-wiped | Medium | N/A | Protocol limitation; blocklist prevents re-registration; codenextremote native client supports full wipe |
...
| 6: Native Agent | 10-15 days | 42-62 days | codenextremote desktop agent binary |
