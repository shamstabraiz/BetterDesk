...
|---|---|---|
| **CRITICAL** | No remote-desktop pipeline through bd-signal — operators only get JPEG snapshots when the RustDesk relay is unavailable. | `codenextremote-agent-client/src-tauri/src/bd_signal.rs` + new `remote-cdap.ejs` |
...
- `codenextremote-agent-client/src-tauri/src/bd_signal.rs` — agent signal/control channel
- `codenextremote-agent-client/src-tauri/src/cdap_client.rs` — CDAP gateway client
