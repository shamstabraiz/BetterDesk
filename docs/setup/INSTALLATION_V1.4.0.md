# 🚀 codenextremote Console v1.4.0 - Installation & Update Guide
...
# 3. Copy new files
cd /path/to/Rustdesk-FreeConsole
sudo cp web/auth.py /opt/codenextremoteConsole/
sudo cp web/app.py /opt/codenextremoteConsole/
sudo cp web/templates/login.html /opt/codenextremoteConsole/templates/
sudo cp web/templates/index.html /opt/codenextremoteConsole/templates/
sudo cp web/static/style.css /opt/codenextremoteConsole/static/
...
3. **Delete credentials file**: `sudo rm /opt/codenextremoteConsole/admin_credentials.txt`
...
```python
cd /opt/codenextremoteConsole/web
python3 -c "
...
# Backup web console
sudo tar -czf /backup/codenextremote-$(date +%Y%m%d).tar.gz /opt/codenextremoteConsole
```
...
**Solutions:**
1. Check credentials file: `sudo cat /opt/codenextremoteConsole/admin_credentials.txt`
...
**Check logs:**
```bash
sudo journalctl -u codenextremote -n 50 --no-pager
```
...
# Restore backup
sudo rm -rf /opt/codenextremoteConsole
sudo cp -r /opt/codenextremoteConsole.backup /opt/codenextremoteConsole
