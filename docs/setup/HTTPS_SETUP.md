...
codenextremote Console supports native HTTPS with TLS certificates, as well as reverse proxy configurations with Caddy or Nginx.
...
```powershell
# Windows (PowerShell)
$cert = New-SelfSignedCertificate -DnsName "codenextremote.local" -CertStoreLocation "cert:\LocalMachine\My" -NotAfter (Get-Date).AddYears(1)
...
# Get certificate (standalone mode - stop codenextremote console first)
sudo systemctl stop codenextremote-console
sudo certbot certonly --standalone -d console.yourdomain.com
sudo systemctl start codenextremote-console
```
...
```bash
# Add to crontab
0 0 1 * * certbot renew --pre-hook "systemctl stop codenextremote-console" --post-hook "systemctl start codenextremote-console"
```
...
Create `/etc/nginx/sites-available/codenextremote`:
...
```bash
sudo ln -s /etc/nginx/sites-available/codenextremote /etc/nginx/sites-enabled/
...
When HTTPS is enabled, codenextremote Console automatically:
...
```powershell
# Windows
New-NetFirewallRule -DisplayName "codenextremote HTTPS" -Direction Inbound -Protocol TCP -LocalPort 5443 -Action Allow
```
...
2. Files are readable by the codenextremote process (check permissions)
...
```bash
# Allow codenextremote to read certificates
...
5. **Ensure the desktop agent (codenextremote Client) can reach the server.** The agent must connect to `/ws/remote-agent/<device_id>` before the browser viewer can stream.

### codenextremote Server (Go) WebSocket Ports

The codenextremote Go server also exposes WebSocket endpoints for RustDesk protocol:
...
The codenextremote ALL-IN-ONE installers (`codenextremote.sh`, `codenextremote.ps1`, `codenextremote-docker.sh`) include a built-in SSL configuration menu accessible via **Option C** in the main menu.
...
Enterprise TLS enables HTTPS/TLS on **all codenextremote ports**, not just the web console:
...
- Certificate SAN (Subject Alternative Name) should include:
  - Domain name (e.g., `codenextremote.example.com`)
  - Public IP address
...
```ini
[Service]
ExecStart=/opt/rustdesk/codenextremote-server \
...
- Check that the Go server started successfully: `journalctl -u codenextremote-server -n 50`
