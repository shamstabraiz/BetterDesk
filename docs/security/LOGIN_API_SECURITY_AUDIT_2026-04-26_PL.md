...
Data: 2026-04-26
Zakres: przepływy logowania codenextremote, obsługa sesji, obsługa tokenów oraz autoryzacja HTTP/WebSocket API w konsoli Node.js i serwerze Go
...
| 3 | Endpointy audit / WS-events bez wyraźnego RBAC | Średnia | **Naprawione** | `codenextremote-server/api/server.go` — `GET /api/audit/events` i `GET /api/ws/events` opakowane w `requirePermission(auth.PermAuditView, ...)`. Wszystkie role wbudowane, które wcześniej korzystały z tych endpointów (super_admin, admin, server_admin, global_admin, operator, viewer) już posiadają `audit.view`; dostęp traci wyłącznie rola `pro` — patrz CHANGELOG. |
| 4 | Wyciek surowych `err.Error()` z handlerów Go | Średnia | **Naprawione** | `codenextremote-server/api/auth_handlers.go` — dziewięć ścieżek 500 zwraca teraz `{"error":"internal error"}` i loguje pełny szczegół po stronie serwera przez `log.Printf`. Kody statusów niezmienione; pozostałe ścieżki bez zmian. |
...
codenextremote ma solidną bazę bezpieczeństwa jak na wieloskładnikową platformę do zdalnego zarządzania. Projekt zawiera już uwierzytelnianie sesyjne dla panelu web, TOTP 2FA, ochronę CSRF, rate limiting, RBAC, klucze API, JWT oraz rozsądną liczbę testów regresyjnych.
...
- auth i routing serwera Go w [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go), [codenextremote-server/api/server.go](../../codenextremote-server/api/server.go), [codenextremote-server/api/client_api_handlers.go](../../codenextremote-server/api/client_api_handlers.go), [codenextremote-server/api/token_handlers.go](../../codenextremote-server/api/token_handlers.go)
- prymitywy auth po stronie Go w [codenextremote-server/auth/jwt.go](../../codenextremote-server/auth/jwt.go), [codenextremote-server/auth/password.go](../../codenextremote-server/auth/password.go) i [codenextremote-server/auth/totp.go](../../codenextremote-server/auth/totp.go)
- testy powiązane w [web-nodejs/tests/auth.routes.test.js](../../web-nodejs/tests/auth.routes.test.js), [web-nodejs/tests/middleware.auth.test.js](../../web-nodejs/tests/middleware.auth.test.js), [web-nodejs/tests/security.middleware.test.js](../../web-nodejs/tests/security.middleware.test.js), [codenextremote-server/auth/password_test.go](../../codenextremote-server/auth/password_test.go), [codenextremote-server/auth/jwt_test.go](../../codenextremote-server/auth/jwt_test.go) i [codenextremote-server/auth/totp_test.go](../../codenextremote-server/auth/totp_test.go)
...
Handlery login oraz login/2fa po stronie Go wymuszają per-IP rate limiting w [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L177) i [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L250).

### 6. Częściowe tokeny 2FA po stronie Go są krótkotrwałe

Go API poprawnie wydaje 5-minutowy częściowy token dla kroku 2FA w [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L213). To dobry domyślny wybór w porównaniu z długowiecznymi tokenami pośrednimi.

### 7. Transport kluczy API jest lepszy niż w starszym modelu

Go API akceptuje klucze API wyłącznie z nagłówka X-API-Key, a nie z query string, zgodnie z [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L801). To zmniejsza ryzyko wycieku do logów, cache i reverse proxy.

### 8. Heartbeat i sysinfo po stronie Go są realnie limitowane

Serwer definiuje i wykorzystuje osobny limiter dla heartbeat/sysinfo w [codenextremote-server/api/server.go](../../codenextremote-server/api/server.go#L76), [codenextremote-server/api/client_api_handlers.go](../../codenextremote-server/api/client_api_handlers.go#L563), [codenextremote-server/api/client_api_handlers.go](../../codenextremote-server/api/client_api_handlers.go#L631) oraz [codenextremote-server/api/client_api_handlers.go](../../codenextremote-server/api/client_api_handlers.go#L708).
...
- permisja audit istnieje: [codenextremote-server/auth/permissions.go](../../codenextremote-server/auth/permissions.go#L36)
- rejestracja trasy audytu: [codenextremote-server/api/server.go](../../codenextremote-server/api/server.go#L201)
- rejestracja websocketów eventów: [codenextremote-server/api/server.go](../../codenextremote-server/api/server.go#L204)
- handler audytu: [codenextremote-server/api/server.go](../../codenextremote-server/api/server.go#L1190)
- typy eventów rozgłaszanych przez event bus: [codenextremote-server/events/bus.go](../../codenextremote-server/events/bus.go)
...
- [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L336)
- [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L498)
- [codenextremote-server/api/auth_handlers.go](../../codenextremote-server/api/auth_handlers.go#L743)
...
- istnieją testy prymitywów auth po stronie Go w [codenextremote-server/auth/password_test.go](../../codenextremote-server/auth/password_test.go), [codenextremote-server/auth/jwt_test.go](../../codenextremote-server/auth/jwt_test.go) oraz [codenextremote-server/auth/totp_test.go](../../codenextremote-server/auth/totp_test.go)
...
Warstwa logowania i API codenextremote jest wyraźnie dojrzalsza niż w przeciętnym autorskim stosie do zdalnego zarządzania. Projekt używa wielu właściwych prymitywów i wzorców. Najważniejsza dalsza praca polega jednak na usunięciu niespójności bezpieczeństwa między różnymi ścieżkami dostępu.
...
Poniższe dwa ustalenia świadomie **nie** są naprawione w tej serii, ponieważ naiwna łatka popsułaby istniejące instalacje codenextremote. Są udokumentowane tutaj, żeby można je było wdrożyć później jako osobne, dobrze przetestowane rollouty.
