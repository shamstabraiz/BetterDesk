# codenextremote 3.0 — Implementation Roadmap

> **Created:** 2026-03-25
> **Status:** Planning phase — awaiting implementation approval
> **Vision:** codenextremote evolves from a remote desktop tool into a full-scale device fleet management ecosystem, comparable to Windows Server domain services but cross-platform and open.
...
**Problem:** Clicking "Chat" in the tray context menu opens the default codenextremote window first, then the user has to click Chat inside it.
...
### 0.3 Rust Compilation Warnings (codenextremote-client) ✅
...
### 0.4 Go Compilation Warnings (codenextremote-server) ✅
...
- `cargo audit` for codenextremote-client: pending (requires cargo-audit installation)
...
### 1.3 Client-Side Login (codenextremote Desktop) ✅
...
- ✅ Client auto-discovers codenextremote server on LAN via mDNS/DNS-SD (`_codenextremote._tcp`) — `discovery/mdns.rs`
...
- User sees: "codenextremote server found: office.example.com — Join?" → login → done.
...
**codenextremote Desktop Client** — `language` field exists in Settings struct but no i18n framework is implemented yet. All UI strings are hardcoded in English.
...
2. Copy `codenextremote-client/src/locales/en.json` → `codenextremote-client/src/locales/{code}.json`
...
### 12.1 README Rebuild
- **Complete rewrite** of `README.md` for codenextremote 3.0 identity
...
### 12.3 codenextremote SDK Documentation
| Document | Description |
|----------|-------------|
| `docs/sdk/OVERVIEW.md` | SDK architecture, supported platforms, capabilities |
| `docs/sdk/PYTHON_SDK.md` | Python SDK reference (`codenextremote-cdap` package) |
| `docs/sdk/NODEJS_SDK.md` | Node.js SDK reference (`codenextremote-cdap` package) |
...
jobs:
  build-windows:
    ...
    steps:
      ...
      - run: cd codenextremote-client && pnpm install && pnpm tauri build
      - uses: actions/upload-artifact@v4
        with:
          name: codenextremote-windows-x64
          path: codenextremote-client/src-tauri/target/release/bundle/nsis/*.exe
  ...
  build-linux:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        with:
          name: codenextremote-linux-x64
          path: codenextremote-client/src-tauri/target/release/bundle/deb/*.deb
  ...
  build-macos:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        with:
          name: codenextremote-macos
          path: codenextremote-client/src-tauri/target/release/bundle/dmg/*.dmg
  ...
  release:
    ...
    steps:
      ...
      - uses: softprops/action-gh-release@v2
        with:
          files: |
            codenextremote-windows-x64/*.exe
            codenextremote-linux-x64/*.deb
            codenextremote-macos/*.dmg
```
...
- Build: `ghcr.io/unitronix/codenextremote-server`, `ghcr.io/unitronix/codenextremote-console`, `ghcr.io/unitronix/codenextremote` (all-in-one)
...
| 🟦 Device Telemetry | codenextremote agent metrics | Device ID, metric names |
...
import { Source, Filter, Output, Flow } from 'codenextremote-cdap-studio';
...
codenextremote as a Linux-native domain controller alternative:
- Deep OS integration via codenextremote SDK/agent
...
- Centralized authentication (codenextremote as identity provider, LDAP/SAML bridge)
