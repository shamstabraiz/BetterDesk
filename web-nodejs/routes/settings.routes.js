/**
 * codenextremote Console - Settings Routes
 */
...
router.get('/api/settings/branding/export', requireAuth, requirePermission('branding.edit'), (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="codenextremote-theme.json"');
        res.json(preset);
    } catch (err) {
        ...
        if (fs.existsSync(themesDir)) {
            for (const file of fs.readdirSync(themesDir)) {
                ...
                try {
                    ...
                    if (data.type === 'codenextremote-theme' && data.branding) {
                        themes.push({
...
router.get('/api/settings/backup', requireAuth, requirePermission('server.config'), async (req, res) => {
    try {
        ...
        const filename = `codenextremote-backup-${new Date().toISOString().slice(0, 10)}.json`;
    ...
    try {
        ...
        if (result.needsServerRestart && (components || []).includes('server')) {
            const svc = updateService.restartService(
                process.platform === 'win32' ? 'codenextremoteServer' : 'codenextremote-server'
            );
