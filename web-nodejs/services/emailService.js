/**
 * codenextremote Console — Email Notification Service
 *
...
async function loadSmtpConfig() {
    ...
    if (process.env.SMTP_HOST) {
        return {
            ...
            from: process.env.SMTP_FROM || 'codenextremote@localhost',
        };
    }
    return null;
}
...
async function sendEmail({ to, subject, text, html }) {
    ...
    try {
        await transporter.sendMail({
            from: config.from || 'codenextremote@localhost',
            to,
            subject,
            text,
            html,
...
async function sendAlertEmail(alert, rule) {
    const subject = `[codenextremote Alert] ${rule.name} — ${alert.severity.toUpperCase()}`;
    const text = [
        `Alert: ${rule.name}`,
    ...
    const html = `
        <div style="font-family: sans-serif; max-width: 600px;">
            <h2 style="color: #e74c3c;">codenextremote Alert: ${escapeHtml(rule.name)}</h2>
