/**
 * codenextremote Console - codenextremote Go Server API Client
 * Full client for the codenextremote Go server REST API (34+ endpoints).
 * Used when serverBackend is set to 'codenextremote'.
 *
...
// Axios instance for codenextremote Go API
// Allow self-signed certificates for local TLS connections
const apiClient = axios.create({
    baseURL: config.codenextremoteApiUrl,
    timeout: config.codenextremoteApiTimeout,
    headers: {
...
apiClient.interceptors.response.use(undefined, async (error) => {
    ...
    if (!_tlsMismatchWarned && error.response?.status === 400) {
        ...
        if (body.includes('HTTP request to an HTTPS server') || body.includes('Client sent an HTTP request')) {
            ...
            console.error('[codenextremote API] ⚠ TLS MISMATCH: Go server has TLS_API=Y enabled on port ' +
                (config.codenextremoteApiUrl || '21114') + ' but this console connects via HTTP.');
            console.error('[codenextremote API]   Fix: remove TLS_API=Y from Go server environment or add -tls-api removal.');
            console.error('[codenextremote API]   The API port must stay HTTP for console↔Go communication. See issue #104.');
        }
    }
    if (error.response?.status === 401 && !_keyReloaded) {
        ...
        try {
            ...
            if (fresh && fresh !== config.codenextremoteApiKey) {
                apiClient.defaults.headers['X-API-Key'] = fresh;
                config.codenextremoteApiKey = fresh;
...
async function getHealth() {
    try {
        ...
        return { ...data, status: 'running', backend: 'codenextremote' };
    } catch (err) {
        return { status: 'unreachable', backend: 'codenextremote', error: err.message };
    }
}
...
async function getAllPeers() {
    try {
        ...
        console.warn('codenextremote API getAllPeers error:', err.message);
        return [];
    }
}
...
async function getOnlinePeers() {
    try {
        ...
        console.warn('codenextremote API getOnlinePeers error:', err.message);
        return [];
    }
}
...
// ========================== Sync (no-op for codenextremote) ======================

/**
 * In codenextremote mode the Go server owns the peer map, so status sync
 * is not needed. This is a no-op kept for interface compatibility.
 */
async function syncOnlineStatus(/* db */) {
    return { synced: 0, skipped: true, reason: 'codenextremote_manages_state' };
}
