/**
 * codenextremote Console - Auth Service
...
function tryGoServerAuth(username, password) {
    const apiUrl = config.codenextremoteApiUrl || config.hbbsApiUrl || 'http://localhost:21114/api';
    let authUrl;
    try {
    ...
    const timeout = config.codenextremoteApiTimeout || 3000;
...
function readAdminCredentialsFile() {
    ...
    if (process.platform === 'win32') {
        candidates.push('C:\\codenextremote', 'C:\\codenextremote\\data',
...
async function generateTotpSetup(userId) {
    ...
    const otpauthUrl = authenticator.keyuri(user.username, 'codenextremote Console', secret);
