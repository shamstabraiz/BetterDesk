/**
 * codenextremote Console - Self-Update Service
 *
 ...
 *
 * GitHub repo:  shamstabraiz/codenextremote
...
const GITHUB_REPO   = process.env.UPDATE_GITHUB_REPO   || 'codenextremote';
const GITHUB_BRANCH = process.env.UPDATE_GITHUB_BRANCH || 'main';
const GITHUB_API    = 'https://api.github.com';
const USER_AGENT    = `codenextremote-Console/${config.appVersion}`;
...
const COMPONENTS = {
    console: {
        ...
        label: 'Web Console',
        localRoot: ROOT_DIR,
        service: IS_WINDOWS ? 'codenextremoteConsole' : 'codenextremote-console',
        autoUpdate: true
    },
    server: {
        prefix: 'codenextremote-server/',
        label: 'Go Server',
        localRoot: path.join(PROJECT_ROOT, 'codenextremote-server'),
        service: IS_WINDOWS ? 'codenextremoteServer' : 'codenextremote-server',
        autoUpdate: false
    },
    agent: {
        prefix: 'codenextremote-agent/',
        label: 'Agent',
        localRoot: null,
        service: IS_WINDOWS ? 'codenextremoteAgent' : 'codenextremote-agent',
        autoUpdate: false
    },
    scripts: {
        // matched by exact file names, not prefix
        files: [
            'codenextremote.sh', 'codenextremote.ps1', 'codenextremote-docker.sh',
...
const EXCLUDE_PATTERNS = [
    ...
    /\.sqlite3$/,
    /\.exe$/,
    /^codenextremote-server\/codenextremote-server/       // compiled binaries
];
...
function detectServerBinaryPath() {
    ...
    try {
        if (IS_WINDOWS) {
            const out = execSync('nssm get codenextremoteServer Application 2>nul', {
                timeout: 5000, stdio: 'pipe'
            }).toString().trim();
            if (out && fs.existsSync(out)) return out;
        } else {
            const raw = execSync(
                'systemctl show codenextremote-server --property=ExecStart --value 2>/dev/null || true',
                { timeout: 5000, stdio: 'pipe' }
            ).toString().trim();
            // ExecStart value may look like: /opt/rustdesk/codenextremote-server --flag ...
    ...
    const candidates = IS_WINDOWS
        ? [
            'C:\\codenextremote\\codenextremote-server.exe',
            'C:\\Program Files\\codenextremote\\codenextremote-server.exe',
            path.join(PROJECT_ROOT, 'codenextremote-server', 'codenextremote-server.exe')
        ]
        : [
            '/opt/rustdesk/codenextremote-server',
            '/opt/codenextremote/codenextremote-server',
            '/usr/local/bin/codenextremote-server',
            path.join(PROJECT_ROOT, 'codenextremote-server', 'codenextremote-server')
        ];
...
async function ensureServerSource(remoteSHA) {
    ...
    try {
        ...
        const srcDir = path.join(tmpDir, 'codenextremote-server');
    ...
    const serverFiles = (tree.tree || []).filter(t =>
        t.path.startsWith('codenextremote-server/') &&
        t.type === 'blob' &&
...
async function buildGoServer() {
    ...
    const binaryName = IS_WINDOWS ? 'codenextremote-server.exe' : 'codenextremote-server';
...
// codenextremote-server/go.mod). The actual point release is selected at
...
function getReleaseBinaryName() {
    const arch = process.arch === 'arm64' ? 'arm64' : 'amd64';
    if (IS_WINDOWS) return `codenextremote-server-windows-${arch}.exe`;
    const os = process.platform === 'darwin' ? 'darwin' : 'linux';
    return `codenextremote-server-${os}-${arch}`;
}
...
async function checkPrebuiltAvailable() {
    try {
        ...
        const altNames = IS_WINDOWS
            ? [binaryName, 'codenextremote-server.exe']
            : [binaryName, `codenextremote-server-${process.platform === 'darwin' ? 'darwin' : 'linux'}`];
    ...
    if (changedData.grouped.server?.length && selectedComponents.includes('server')) {
        ...
        if (wantsCompile && goAvailable) {
            ...
            if (!downloadResult || !downloadResult.success) {
                const binaryName = IS_WINDOWS ? 'codenextremote-server.exe' : 'codenextremote-server-linux-amd64';
                const repoPath = `codenextremote-server/${binaryName}`;
                try {
                    ...
                    if (data && data.length > 1024 * 1024) {
                        const serverDir = COMPONENTS.server.localRoot;
                        fs.mkdirSync(serverDir, { recursive: true });
                        const outName = IS_WINDOWS ? 'codenextremote-server.exe' : 'codenextremote-server';
