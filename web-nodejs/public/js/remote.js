/**
 * codenextremote Console - Remote Desktop Session Manager
...
(function () {
    ...
    function createSession(deviceId, deviceName) {
        ...
        const userName = (window.codenextremote.user && (window.codenextremote.user.display_name || window.codenextremote.user.username)) || 'codenextremote Web';
        session.client = createTransportClient(session.canvas, {
            deviceId: deviceId,
            serverPubKey: window.codenextremote.serverPubKey || '',
            myName: userName,
            scaleMode: 'fit',
            fps: 30,
        ...
        if (session.mediaRecorder && session.mediaRecorder.state === 'recording') {
            ...
            try {
                ...
                session.mediaRecorder.onstop = () => {
                    ...
                    a.href = url;
                    a.download = 'codenextremote-' + session.deviceId + '-' + Date.now() + '.webm';
                    a.click();
