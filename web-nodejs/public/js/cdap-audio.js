/**
 * codenextremote Console — CDAP Audio Stream Widget
...
(function () {
    ...
    function openAudio(deviceId, widgetId, opts) {
        ...
        if (statusEl) {
            statusEl.textContent = window.codenextremote?.t?.('cdap.audio_connecting') || 'Connecting...';
    ...
    function handleMessage(session, msg) {
        switch (msg.type) {
            case 'ready':
                ...
                if (session.statusEl) {
                    session.statusEl.textContent = window.codenextremote?.t?.('cdap.audio_streaming') || 'Streaming';
    ...
    function setDisconnected(session) {
        ...
        if (session.statusEl) {
            session.statusEl.textContent = window.codenextremote?.t?.('cdap.disconnected') || 'Disconnected';
