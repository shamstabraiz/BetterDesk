...
#===============================================================================
#
#   codenextremote Console Manager v3.0.0
...
#
#   Usage: 
#     Interactive: sudo ./codenextremote.sh
#     Auto mode:   sudo ./codenextremote.sh --auto
#     PostgreSQL:  sudo ./codenextremote.sh --auto --postgresql
#
#===============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --flask)
            ...
            ;;
        --help|-h)
            echo "codenextremote Console Manager v$VERSION"
            echo ""
            echo "Usage: sudo ./codenextremote.sh [OPTIONS]"
            echo ""
            echo "Options:"
            ...
            echo "  POSTGRESQL_USER=...     PostgreSQL username (default: codenextremote)"
            echo "  POSTGRESQL_PASS=...     PostgreSQL password (auto-generated if empty)"
            echo "  POSTGRESQL_DB=...       PostgreSQL database (default: codenextremote)"
...
GO_SERVER_SOURCE="$SCRIPT_DIR/codenextremote-server"
...
POSTGRESQL_USER="${POSTGRESQL_USER:-codenextremote}"
POSTGRESQL_PASS="${POSTGRESQL_PASS:-}"
POSTGRESQL_DB="${POSTGRESQL_DB:-codenextremote}"
...
COMMON_CONSOLE_PATHS=(
    "/opt/codenextremoteConsole"
    "/opt/codenextremote"
    "/var/lib/codenextremote"
    "$HOME/codenextremoteConsole"
)
...
# Logging
LOG_FILE="/tmp/codenextremote_$(date +%Y%m%d_%H%M%S).log"
...
# Gracefully stop all codenextremote services with proper cleanup
graceful_stop_services() {
    print_step "Stopping services gracefully..."
    
    # New Go services (primary)
    local services=("codenextremote-console" "codenextremote-server")
    # Legacy services (for migration)
    local legacy_services=("codenextremote" "rustdesksignal" "rustdeskrelay" "codenextremote-api" "codenextremote-go")
    ...
    kill_stale_processes "codenextremote-server"
...
start_services_with_verification() {
    ...
    # Enable services
    systemctl enable codenextremote-server codenextremote-console 2>/dev/null || true
    
    # Start Go server (signal + relay + API in one binary)
    print_info "Starting codenextremote-server (Go)..."
    systemctl start codenextremote-server
    sleep 3
    
    if ! verify_service_health "codenextremote-server" "21116" 10; then
        print_error "Failed to start codenextremote-server"
        print_info "Service state: $(systemctl show codenextremote-server --property=ActiveState --value 2>/dev/null)"
        print_info "Run: journalctl -u codenextremote-server -n 50 --no-pager"
        return 1
    fi
    print_success "codenextremote-server started and healthy"
    ...
    if ! verify_service_health "codenextremote-server" "21117" 5; then
        print_warning "Relay port 21117 may not be ready yet"
    fi
    
    # Start Node.js console
    print_info "Starting codenextremote-console (Node.js)..."
    systemctl start codenextremote-console
    sleep 2
    
    if ! verify_service_health "codenextremote-console" "5000" 10; then
        print_warning "Web console may not be running correctly"
        local console_state
        console_state=$(systemctl show codenextremote-console --property=ActiveState --value 2>/dev/null)
        if [ "$console_state" = "failed" ]; then
            ...
            print_info "Run: journalctl -u codenextremote-console -n 50 --no-pager"
        fi
        # Don't fail for console - it's not critical
    else
        print_success "codenextremote-console started and healthy"
    fi
...
detect_installation() {
    ...
    if [ -d "$RUSTDESK_PATH" ]; then
        ...
        if [ -f "$RUSTDESK_PATH/codenextremote-server" ]; then
            BINARIES_OK=true
            SERVER_TYPE="go"
    ...
    if systemctl is-active --quiet codenextremote-server 2>/dev/null; then
        HBBS_RUNNING=true
    ...
    if systemctl is-active --quiet codenextremote-console 2>/dev/null || \
       systemctl is-active --quiet codenextremote 2>/dev/null; then
...
auto_detect_paths() {
    ...
    if [ -n "$RUSTDESK_PATH" ]; then
        if [ -d "$RUSTDESK_PATH" ] && { [ -f "$RUSTDESK_PATH/codenextremote-server" ] || [ -f "$RUSTDESK_PATH/hbbs" ] || [ -f "$RUSTDESK_PATH/hbbs-v8-api" ]; }; then
    ...
    if [ -z "$RUSTDESK_PATH" ]; then
        for path in "${COMMON_RUSTDESK_PATHS[@]}"; do
            if [ -d "$path" ] && { [ -f "$path/codenextremote-server" ] || [ -f "$path/hbbs" ] || [ -f "$path/hbbs-v8-api" ]; }; then
                RUSTDESK_PATH="$path"
    ...
    if [ -z "$RUSTDESK_PATH" ]; then
        RUSTDESK_PATH="/opt/codenextremote"
    ...
    if [ -z "$CONSOLE_PATH" ]; then
        CONSOLE_PATH="/opt/codenextremoteConsole"
    fi
...
configure_paths() {
    ...
    case $choice in
        ...
        3)
            echo ""
            echo -n "Enter Console path (e.g., /opt/codenextremoteConsole): "
            read -r new_path
            if [ -n "$new_path" ]; then
                if [ -d "$new_path" ]; then
...
print_status() {
    ...
    if [ "${SERVER_TYPE:-}" = "go" ] || systemctl is-active --quiet codenextremote-server 2>/dev/null; then
        local go_state
        go_state=$(systemctl show codenextremote-server --property=ActiveState --value 2>/dev/null || echo "unknown")
        case "$go_state" in
            active)
                echo -e "  codenextremote Server (Go): ${GREEN}● Active${NC} (Signal + Relay + API)"
                ;;
            failed)
                echo -e "  codenextremote Server (Go): ${RED}✗ Failed${NC} (check: journalctl -u codenextremote-server -n 30)"
                ;;
            activating)
                echo -e "  codenextremote Server (Go): ${YELLOW}◌ Starting...${NC}"
                ;;
            *)
                echo -e "  codenextremote Server (Go): ${RED}○ Inactive${NC} ($go_state)"
                ;;
        esac
    else
        # Legacy Rust servers
    ...
    local console_state
    console_state=$(systemctl show codenextremote-console --property=ActiveState --value 2>/dev/null || echo "unknown")
    case "$console_state" in
        active)
            echo -e "  Web Console:   ${GREEN}● Active${NC}"
            ;;
        failed)
            echo -e "  Web Console:   ${RED}✗ Failed${NC} (check: journalctl -u codenextremote-console -n 30)"
            ;;
        activating)
...
compile_go_server() {
    print_step "Compiling codenextremote Go server..."
    ...
    rm -f codenextremote-server codenextremote-server-linux-*
    
    # Build
    print_info "Building codenextremote server for $ARCH_NAME..."
    local output_name="codenextremote-server"
...
verify_go_binary() {
    ...
    if [ -z "$binary_path" ]; then
        binary_path="$GO_SERVER_SOURCE/codenextremote-server"
    fi
    
    if [ ! -f "$binary_path" ]; then
        # Check installed location
        binary_path="$RUSTDESK_PATH/codenextremote-server"
    fi
...
verify_binaries() {
    print_step "Verifying codenextremote server..."
    ...
    if [ -f "$GO_SERVER_SOURCE/codenextremote-server" ]; then
        if verify_go_binary "$GO_SERVER_SOURCE/codenextremote-server"; then
            local size=$(du -h "$GO_SERVER_SOURCE/codenextremote-server" | cut -f1)
    ...
    if [ -f "$RUSTDESK_PATH/codenextremote-server" ]; then
        if verify_go_binary "$RUSTDESK_PATH/codenextremote-server"; then
            local size=$(du -h "$RUSTDESK_PATH/codenextremote-server" | cut -f1)
    ...
    if [ "$found" = false ]; then
        print_warning "No codenextremote server binary found"
...
setup_postgresql_database() {
    print_step "Setting up PostgreSQL database for codenextremote..."
...
migrate_sqlite_to_postgresql() {
    ...
    local migrate_bin=""
    if [ -f "$SCRIPT_DIR/codenextremote-server/tools/migrate/migrate-linux-amd64" ]; then
        migrate_bin="$SCRIPT_DIR/codenextremote-server/tools/migrate/migrate-linux-amd64"
    elif [ -f "$SCRIPT_DIR/tools/migrate/migrate-linux-amd64" ]; then
        migrate_bin="$SCRIPT_DIR/tools/migrate/migrate-linux-amd64"
    elif [ -f "/opt/codenextremote-go/migrate" ]; then
        migrate_bin="/opt/codenextremote-go/migrate"
    fi
    
    # Try to compile migration tool from source if not found or outdated
    if [ -z "$migrate_bin" ] && command -v go &>/dev/null; then
        local migrate_src="$SCRIPT_DIR/codenextremote-server/tools/migrate"
        if [ -d "$migrate_src" ]; then
            print_info "Compiling migration tool from source..."
            if (cd "$SCRIPT_DIR/codenextremote-server" && go build -o "tools/migrate/migrate-linux-amd64" ./tools/migrate/) 2>&1; then
    ...
    if ! "$migrate_bin" -mode backup -src /dev/null 2>&1 | grep -qv "flag provided but not defined"; then
        if "$migrate_bin" -mode backup -src /dev/null 2>&1 | grep -q "flag provided but not defined"; then
            ...
            print_info "Rebuild with: cd codenextremote-server && go build -o tools/migrate/migrate-linux-amd64 ./tools/migrate/"
            return 0
        fi
    fi
...
install_nodejs_console() {
    ...
    local npm_log="/tmp/codenextremote_npm_install.log"
...
# codenextremote Node.js Console Configuration
PORT=5000
HOST=0.0.0.0
NODE_ENV=production
...
# Server backend (codenextremote = Go server, rustdesk = legacy Rust)
...
HTTPS_ENABLED=false
HTTPS_PORT=5443
SSL_CERT_PATH=$RUSTDESK_PATH/ssl/codenextremote.crt
SSL_KEY_PATH=$RUSTDESK_PATH/ssl/codenextremote.key
SSL_CA_PATH=
...
install_binaries() {
    ...
    print_step "Installing codenextremote Go Server..."
    ...
    local go_binary="$GO_SERVER_SOURCE/codenextremote-server"
    ...
    # Copy binary
    cp "$go_binary" "$RUSTDESK_PATH/codenextremote-server"
    chmod +x "$RUSTDESK_PATH/codenextremote-server"
    
    print_success "codenextremote Go Server v$VERSION installed"
...
migrate_console() {
    ...
    systemctl stop codenextremote 2>/dev/null || true
...
generate_ssl_certificates() {
    ...
    if [ -f "$ssl_dir/codenextremote.crt" ] && [ -f "$ssl_dir/codenextremote.key" ]; then
    ...
    openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
        -keyout "$ssl_dir/codenextremote.key" \
        -out "$ssl_dir/codenextremote.crt" \
        -subj "/CN=$cn/O=codenextremote/C=PL" \
    ...
    chmod 600 "$ssl_dir/codenextremote.key"
    chmod 644 "$ssl_dir/codenextremote.crt"
    
    # Also symlink to console SSL directory for Node.js
    if [ -d "$CONSOLE_PATH" ]; then
        local console_ssl="$CONSOLE_PATH/ssl"
        mkdir -p "$console_ssl"
        ln -sf "$ssl_dir/codenextremote.crt" "$console_ssl/codenextremote.crt" 2>/dev/null || \
            cp -f "$ssl_dir/codenextremote.crt" "$console_ssl/codenextremote.crt"
        ln -sf "$ssl_dir/codenextremote.key" "$console_ssl/codenextremote.key" 2>/dev/null || \
            cp -f "$ssl_dir/codenextremote.key" "$console_ssl/codenextremote.key"
        ...
        if [ -f "$env_file" ]; then
            ...
            sed -i "s|^SSL_CERT_PATH=.*|SSL_CERT_PATH=$ssl_dir/codenextremote.crt|" "$env_file"
            sed -i "s|^SSL_KEY_PATH=.*|SSL_KEY_PATH=$ssl_dir/codenextremote.key|" "$env_file"
            ...
            if grep -q '^NODE_EXTRA_CA_CERTS=' "$env_file" 2>/dev/null; then
                sed -i "s|^NODE_EXTRA_CA_CERTS=.*|NODE_EXTRA_CA_CERTS=$ssl_dir/codenextremote.crt|" "$env_file"
            else
                echo "NODE_EXTRA_CA_CERTS=$ssl_dir/codenextremote.crt" >> "$env_file"
            fi
    ...
    print_info "Certificate: $ssl_dir/codenextremote.crt"
    print_info "Private key: $ssl_dir/codenextremote.key"
...
setup_services() {
    ...
    if [ "$server_ip" = "127.0.0.1" ] || [[ "$server_ip" == 10.* ]] || [[ "$server_ip" == 192.168.* ]] || [[ "$server_ip" == 172.1[6-9].* ]] || [[ "$server_ip" == 172.2[0-9].* ]] || [[ "$server_ip" == 172.3[0-1].* ]]; then
        ...
        echo ""
        echo -e "  ${YELLOW}Example: RELAY_SERVERS=YOUR.PUBLIC.IP sudo ./codenextremote.sh${NC}"
        echo ""
    fi
    ...
    if [ -f "$ssl_dir/codenextremote.crt" ] && [ -f "$ssl_dir/codenextremote.key" ]; then
        ...
        cert_issuer=$(openssl x509 -in "$ssl_dir/codenextremote.crt" -noout -issuer 2>/dev/null | sed 's/^issuer[= ]*//' || echo "")
        cert_subject=$(openssl x509 -in "$ssl_dir/codenextremote.crt" -noout -subject 2>/dev/null | sed 's/^subject[= ]*//' || echo "")
        if [ -n "$cert_issuer" ] && [ "$cert_issuer" = "$cert_subject" ]; then
            tls_is_selfsigned=true
        elif echo "$cert_subject" | grep -qi "codenextremote"; then
            tls_is_selfsigned=true
        fi
        ...
        tls_arg="-tls-cert $ssl_dir/codenextremote.crt -tls-key $ssl_dir/codenextremote.key -tls-signal -tls-relay"
    ...
    # codenextremote Go Server (single binary replacing hbbs+hbbr)
    ...
    cat > /etc/systemd/system/codenextremote-server.service << EOF
[Unit]
Description=codenextremote Go Server v$VERSION (Signal + Relay + API)
...
ExecStart=$RUSTDESK_PATH/codenextremote-server -mode all -relay-servers $server_ip $systemd_db_arg -key-file $RUSTDESK_PATH/id_ed25519 -api-port $API_PORT $init_admin_arg $tls_arg
Restart=always
RestartSec=5
LimitNOFILE=1000000
...
EOF
    ...
    print_success "Created codenextremote-server.service (Go)"
    ...
    # Remove legacy Flask codenextremote-api.service (deprecated in v2.3.0)
    if [ -f /etc/systemd/system/codenextremote-api.service ]; then
        systemctl stop codenextremote-api 2>/dev/null || true
        systemctl disable codenextremote-api 2>/dev/null || true
        rm -f /etc/systemd/system/codenextremote-api.service
        print_info "Removed legacy codenextremote-api.service (Flask)"
    fi
    
    # Remove stale codenextremote-go.service (manual installs, wrong credentials)
    if [ -f /etc/systemd/system/codenextremote-go.service ]; then
        systemctl stop codenextremote-go 2>/dev/null || true
        systemctl disable codenextremote-go 2>/dev/null || true
        rm -f /etc/systemd/system/codenextremote-go.service
        print_info "Removed stale codenextremote-go.service"
    fi
...
Environment=SSL_CERT_PATH=$ssl_dir/codenextremote.crt
Environment=SSL_KEY_PATH=$ssl_dir/codenextremote.key"
        fi
        ...
        cat > /etc/systemd/system/codenextremote-console.service << EOF
[Unit]
Description=codenextremote Web Console (Node.js)
Documentation=https://github.com/shamstabraiz/Rustdesk-FreeConsole
After=network.target codenextremote-server.service postgresql.service
...
SyslogIdentifier=codenextremote-console
...
Environment=SERVER_BACKEND=codenextremote
Environment=PORT=5000
Environment=HOST=0.0.0.0
Environment=API_HOST=0.0.0.0
$tls_env
$([ "$tls_is_selfsigned" = true ] && echo "Environment=NODE_EXTRA_CA_CERTS=$ssl_dir/codenextremote.crt" || true)
Restart=always
RestartSec=5
...
WantedBy=multi-user.target
EOF
        print_success "Created codenextremote-console.service (Node.js)"
        
        # Remove legacy codenextremote.service if exists
        if [ -f /etc/systemd/system/codenextremote.service ]; then
            systemctl stop codenextremote 2>/dev/null || true
            systemctl disable codenextremote 2>/dev/null || true
            rm -f /etc/systemd/system/codenextremote.service
            print_info "Removed legacy codenextremote.service"
        fi
    fi
    ...
    print_info "Services: codenextremote-server, codenextremote-console"
}
...
#===============================================================================
# codenextremote Minimal Installation (Go server only, no web console)
#===============================================================================
...
do_install_minimal() {
    ...
    print_info "codenextremote Minimal installs the Go server binary only."
    ...
    if [ "$INSTALL_STATUS" = "complete" ]; then
        print_warning "codenextremote is already installed!"
        if [ "$AUTO_MODE" = false ]; then
    ...
    if command -v ufw >/dev/null 2>&1; then
        ufw allow 21114/tcp comment "codenextremote API" 2>/dev/null || true
        ufw allow 21115/tcp comment "codenextremote NAT" 2>/dev/null || true
        ufw allow 21116/tcp comment "codenextremote Signal TCP" 2>/dev/null || true
        ufw allow 21116/udp comment "codenextremote Signal UDP" 2>/dev/null || true
        ufw allow 21117/tcp comment "codenextremote Relay" 2>/dev/null || true
        ufw allow 21118/tcp comment "codenextremote WS Signal" 2>/dev/null || true
        ufw allow 21119/tcp comment "codenextremote WS Relay" 2>/dev/null || true
    fi
    
    # Start server
    print_step "Starting codenextremote server..."
    systemctl daemon-reload
    systemctl start codenextremote-server.service 2>/dev/null || true
    systemctl enable codenextremote-server.service 2>/dev/null || true
    
    sleep 3
    
    # Verify
    if systemctl is-active --quiet codenextremote-server.service; then
        print_success "codenextremote server is running"
    else
        print_error "codenextremote server failed to start"
        journalctl -u codenextremote-server.service --no-pager -n 20
        return 1
    fi
...
setup_services_minimal() {
    print_step "Setting up codenextremote server service (Minimal mode)..."
    
    local GO_BINARY_PATH="$INSTALL_DIR/codenextremote-server"
    ...
    local TLS_CERT_PATH="$SSL_DIR/codenextremote.crt"
    local TLS_KEY_PATH="$SSL_DIR/codenextremote.key"
    ...
    for old_svc in rustdesksignal rustdeskrelay codenextremote-api codenextremote-go codenextremote-console; do
    ...
    cat > /etc/systemd/system/codenextremote-server.service <<EOF
[Unit]
Description=codenextremote Server (Minimal)
...
StandardOutput=journal
StandardError=journal
SyslogIdentifier=codenextremote-server
...
EOF
    ...
    systemctl daemon-reload
    print_success "codenextremote server service created (Minimal mode)"
}
...
do_install() {
    ...
    echo ""
    print_info "Starting codenextremote Console v$VERSION installation..."
    echo ""
    ...
    if [ "$go_state" != "active" ]; then
        print_error "codenextremote-server is $go_state (expected: active)"
        print_info "Debug: journalctl -u codenextremote-server -n 30 --no-pager"
        install_ok=false
    fi
    ...
    if [ "$console_state" != "active" ]; then
        print_warning "codenextremote-console is $console_state (expected: active)"
        print_info "Debug: journalctl -u codenextremote-console -n 30 --no-pager"
        install_ok=false
    fi
    ...
    if [ -f "$RUSTDESK_PATH/ssl/codenextremote.crt" ] && [ -f "$RUSTDESK_PATH/ssl/codenextremote.key" ]; then
...
do_update() {
    ...
    if [ "$INSTALL_STATUS" = "none" ]; then
        print_error "codenextremote is not installed!"
...
do_repair() {
    ...
    echo ""
    echo "  1. 🔧 Repair binaries (replace with codenextremote)"
...
repair_binaries() {
    print_step "Repairing codenextremote Go Server..."
    ...
    if [ -f "$RUSTDESK_PATH/codenextremote-server" ]; then
        cp "$RUSTDESK_PATH/codenextremote-server" "$RUSTDESK_PATH/codenextremote-server.backup.$(date +%Y%m%d%H%M%S)" 2>/dev/null || true
    fi
    ...
    if lsof "$RUSTDESK_PATH/codenextremote-server" 2>/dev/null | grep -q .; then
        print_error "codenextremote-server binary is still in use!"
        kill_stale_processes "codenextremote-server"
        sleep 2
    fi
...
repair_services() {
    ...
    for svc in codenextremote-server codenextremote-console rustdesksignal rustdeskrelay codenextremote; do
    ...
    if [ ! -f "$RUSTDESK_PATH/codenextremote-server" ]; then
        print_error "codenextremote-server binary not found at $RUSTDESK_PATH/codenextremote-server"
    ...
    if ! start_services_with_verification; then
        ...
        for svc in codenextremote-server codenextremote-console; do
...
repair_permissions() {
    ...
    chmod +x "$RUSTDESK_PATH/codenextremote-server" 2>/dev/null || true
...
do_validate() {
    ...
    echo -n "  codenextremote Server (Go): "
    if [ -x "$RUSTDESK_PATH/codenextremote-server" ]; then
    ...
    elif systemctl is-enabled --quiet codenextremote-server 2>/dev/null; then
        echo -e "${YELLOW}○ Enabled but inactive${NC}"
        warnings=$((warnings + 1))
    elif systemctl list-unit-files codenextremote-server.service &>/dev/null 2>&1; then
    ...
    echo -n "  codenextremote-console (Node.js): "
    if systemctl is-active --quiet codenextremote-console 2>/dev/null; then
        echo -e "${GREEN}● Active${NC}"
    elif systemctl is-active --quiet codenextremote 2>/dev/null; then
        echo -e "${GREEN}● Active (legacy name)${NC}"
    elif systemctl is-enabled --quiet codenextremote-console 2>/dev/null; then
        echo -e "${YELLOW}○ Enabled but inactive${NC}"
        warnings=$((warnings + 1))
    else
        echo -e "${RED}○ Disabled${NC}"
...
do_backup_silent() {
    local backup_name="codenextremote_backup_$(date +%Y%m%d_%H%M%S)"
...
PYEOF
    ...
    if [ "$success" = "true" ]; then
        ...
        if systemctl is-active codenextremote-console &>/dev/null; then
            print_info "Restarting codenextremote-console..."
            systemctl restart codenextremote-console 2>/dev/null || true
            sleep 2
        fi
...
do_rebuild_go_server() {
    ...
    if [ "$INSTALL_STATUS" = "none" ]; then
        print_warning "codenextremote is not installed. Binary will be compiled but not deployed."
    ...
    local new_binary="$GO_SERVER_SOURCE/codenextremote-server"
    ...
    local installed_binary="$RUSTDESK_PATH/codenextremote-server"
    local ts
    ...
    if systemctl is-active --quiet codenextremote-server 2>/dev/null; then
        ...
        journalctl -u codenextremote-server -n 5 --no-pager 2>/dev/null || true
    else
        ...
        if [ -f "${installed_binary}.backup.${ts}" ]; then
            ...
            chmod +x "$installed_binary"
            systemctl start codenextremote-server 2>/dev/null || true
            sleep 2
            if systemctl is-active --quiet codenextremote-server 2>/dev/null; then
                print_success "Rollback successful — previous binary restored"
            else
                print_error "Rollback also failed. Check: journalctl -u codenextremote-server -n 50"
            fi
        else
            print_error "No backup to rollback to. Check: journalctl -u codenextremote-server -n 50"
        fi
    fi
...
do_build_legacy_rust() {
    ...
    local build_dir="/tmp/codenextremote_build_$$"
    mkdir -p "$build_dir"
    cd "$build_dir"
    ...
    print_step "Applying codenextremote modifications..."
    ...
    if command -v ufw &>/dev/null && ufw status 2>/dev/null | grep -q "active"; then
        ...
        for port in $required_ports; do
            ...
            if ! ufw status 2>/dev/null | grep -qE "^${port}[/ ]"; then
                if [ "$port" = "21116" ]; then
                    ufw allow 21116/tcp comment "codenextremote ID Server TCP" 2>/dev/null && created=$((created + 1))
                    ufw allow 21116/udp comment "codenextremote ID Server UDP" 2>/dev/null && created=$((created + 1))
                    total=$((total + 1))
                else
                    ufw allow "${port}/tcp" comment "codenextremote port ${port}" 2>/dev/null && created=$((created + 1))
                fi
            fi
        done
...
do_diagnostics() {
    ...
    if systemctl list-unit-files codenextremote-server.service &>/dev/null 2>&1; then
        echo -e "${CYAN}--- codenextremote-server (Go) ---${NC}"
        journalctl -u codenextremote-server -n 10 --no-pager 2>/dev/null || echo "No logs found"
    else
    ...
    echo ""
    echo -e "${CYAN}--- codenextremote-console (Node.js) ---${NC}"
    journalctl -u codenextremote-console -n 10 --no-pager 2>/dev/null || \
        journalctl -u codenextremote -n 10 --no-pager 2>/dev/null || echo "No logs found"
    ...
    local port_issues=0
    local port_defs=(
        "21114:TCP:codenextremote-serv|codenextremote-server|hbbs:API Server"
        "21115:TCP:codenextremote-serv|codenextremote-server|hbbs:NAT Test"
        "21116:TCP:codenextremote-serv|codenextremote-server|hbbs:ID Server (TCP)"
        "21116:UDP:codenextremote-serv|codenextremote-server|hbbs:ID Server (UDP)"
        "21117:TCP:codenextremote-serv|codenextremote-server|hbbr:Relay Server"
    ...
    local api_scheme="http"
    if systemctl cat codenextremote-server.service 2>/dev/null | grep -qE '\-tls-api'; then
        api_use_tls=true
        api_scheme="https"
    fi
    ...
    if [ "$api_use_tls" = true ]; then
        if curl -skfo /dev/null --connect-timeout 3 "https://127.0.0.1:${api_port}/api/health" 2>/dev/null; then
            ...
            if curl -sfo /dev/null --connect-timeout 3 "http://127.0.0.1:${api_port}/api/health" 2>/dev/null; then
                ...
                echo -e "  ${YELLOW}Tip: Check codenextremote-server logs: journalctl -u codenextremote-server -n 20${NC}"
            fi
        fi
    else
        if curl -sfo /dev/null --connect-timeout 3 "http://127.0.0.1:${api_port}/api/health" 2>/dev/null; then
    ...
    if [ "$api_use_tls" = true ]; then
        ...
        if [ -f /etc/systemd/system/codenextremote-console.service ]; then
            console_api_url=$(grep 'BETTERDESK_API_URL=' /etc/systemd/system/codenextremote-console.service 2>/dev/null | tail -1 | sed 's/.*BETTERDESK_API_URL=//')
        fi
...
do_uninstall() {
    ...
    print_warning "This operation will remove codenextremote Console!"
    echo ""
    ...
    systemctl stop codenextremote-server codenextremote-console 2>/dev/null || true
    systemctl disable codenextremote-server codenextremote-console 2>/dev/null || true
    # Stop legacy Rust services if they exist
    systemctl stop rustdesksignal rustdeskrelay codenextremote codenextremote-api codenextremote-go 2>/dev/null || true
    systemctl disable rustdesksignal rustdeskrelay codenextremote codenextremote-api codenextremote-go 2>/dev/null || true
    
    print_step "Removing service files..."
    # Remove Go services
    rm -f /etc/systemd/system/codenextremote-server.service
    rm -f /etc/systemd/system/codenextremote-console.service
    ...
    print_success "codenextremote has been uninstalled"
    press_enter
}
...
do_configure_ssl() {
    ...
    if [ ! -f "$CONSOLE_PATH/.env" ]; then
        ...
        print_info "Please install codenextremote first (option 1)"
        press_enter
        return
    fi
    
    echo -e "  ${WHITE}Configure SSL/TLS certificates for codenextremote Console.${NC}"
    ...
    case "${ssl_choice:-1}" in
        1)
            # Let's Encrypt
            echo ""
            read -p "Enter your domain name (e.g., codenextremote.example.com): " domain
            if [ -z "$domain" ]; then
            ...
            if ! crontab -l 2>/dev/null | grep -q "certbot renew"; then
                (crontab -l 2>/dev/null; echo "0 3 * * * certbot renew --quiet --post-hook 'systemctl restart codenextremote-console codenextremote-server'") | crontab -
        ...
        3)
            ...
            openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
                ...
                openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
                    -keyout "$ssl_dir/codenextremote.key" \
                    -out "$ssl_dir/codenextremote.crt" \
                    -subj "/CN=$cn/O=codenextremote/C=PL" 2>&1
            }
            ...
            sed -i "s|^HTTPS_ENABLED=.*|HTTPS_ENABLED=true|" "$CONSOLE_PATH/.env"
            sed -i "s|^SSL_CERT_PATH=.*|SSL_CERT_PATH=$ssl_dir/codenextremote.crt|" "$CONSOLE_PATH/.env"
            sed -i "s|^SSL_KEY_PATH=.*|SSL_KEY_PATH=$ssl_dir/codenextremote.key|" "$CONSOLE_PATH/.env"
            sed -i "s|^HTTP_REDIRECT_HTTPS=.*|HTTP_REDIRECT_HTTPS=true|" "$CONSOLE_PATH/.env"
            
            # Configure NODE_EXTRA_CA_CERTS for self-signed
            if grep -q '^NODE_EXTRA_CA_CERTS=' "$CONSOLE_PATH/.env" 2>/dev/null; then
                sed -i "s|^NODE_EXTRA_CA_CERTS=.*|NODE_EXTRA_CA_CERTS=$ssl_dir/codenextremote.crt|" "$CONSOLE_PATH/.env"
            else
                echo "NODE_EXTRA_CA_CERTS=$ssl_dir/codenextremote.crt" >> "$CONSOLE_PATH/.env"
            fi
            
            # Configure Go server with TLS for signal/relay
            local go_svc_file="/etc/systemd/system/codenextremote-server.service"
            if [ -f "$go_svc_file" ]; then
                ...
                sed -i "s|\(ExecStart=.*codenextremote-server[^$]*\)|\1 -tls-cert $ssl_dir/codenextremote.crt -tls-key $ssl_dir/codenextremote.key -tls-signal -tls-relay|" "$go_svc_file"
            fi
        ...
        5)
            ...
            openssl req -x509 -nodes -days 3650 -newkey rsa:4096 \
                -keyout "$ssl_dir/codenextremote.key" \
                -out "$ssl_dir/codenextremote.crt" \
                -subj "/CN=$cn/O=codenextremote Enterprise/C=PL" \
                -addext "subjectAltName=$san_list" 2>&1 || {
                # Fallback for older openssl
                openssl req -x509 -nodes -days 3650 -newkey rsa:4096 \
                    -keyout "$ssl_dir/codenextremote.key" \
                    -out "$ssl_dir/codenextremote.crt" \
                    -subj "/CN=$cn/O=codenextremote Enterprise/C=PL" 2>&1
            }
            ...
            if [ -f "$go_svc_file" ]; then
                ...
                sed -i "s|\(ExecStart=.*codenextremote-server[^$]*\)|\1 -tls-cert $ssl_dir/codenextremote.crt -tls-key $ssl_dir/codenextremote.key -tls-signal -tls-relay -tls-api|" "$go_svc_file"
            fi
            ...
            print_info "  1. Import $ssl_dir/codenextremote.crt as trusted CA"
    ...
    if [ "${ssl_choice:-1}" = "5" ]; then
        ...
        local svc_file="/etc/systemd/system/codenextremote-console.service"
        if [ -f "$svc_file" ]; then
    ...
    echo ""
    if confirm "Restart codenextremote to apply changes?"; then
        systemctl restart codenextremote-server codenextremote-console 2>/dev/null || true
        print_success "codenextremote services restarted"
    fi
...
do_migrate_database() {
    ...
    local arch=$(uname -m)
    local search_paths=(
        "$SCRIPT_DIR/codenextremote-server/tools/migrate/migrate-linux-amd64"
        "$SCRIPT_DIR/tools/migrate/migrate-linux-amd64"
        "$RUSTDESK_PATH/migrate"
        "/usr/local/bin/codenextremote-migrate"
    )
    ...
    if [ -z "$migrate_bin" ]; then
        ...
        print_info "Expected at: $SCRIPT_DIR/codenextremote-server/tools/migrate/migrate-linux-amd64"
        print_info "Build it with: cd codenextremote-server && go build -o tools/migrate/migrate-linux-amd64 ./tools/migrate/"
        press_enter
        return
    fi
    ...
    echo ""
    echo -e "  ${WHITE}Migrate databases between different codenextremote components.${NC}"
    echo ""
    ...
    echo -e "  ${GREEN}3.${NC} SQLite → PostgreSQL  Migrate codenextremote Go SQLite to PostgreSQL"
    ...
    case $mig_choice in
        ...
        3)
            ...
            if [ $? -eq 0 ]; then
                ...
                print_info "Update your codenextremote Go server config: DB_URL=$pg_uri"
            else
                print_error "Migration failed. Check the output above for details."
            fi
            ;;
