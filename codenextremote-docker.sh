...
#
#   Usage: ./codenextremote-docker.sh
#
#===============================================================================
...
DATA_DIR="${DATA_DIR:-}"
BACKUP_DIR="${BACKUP_DIR:-/opt/codenextremote-backups}"
...
COMMON_DATA_PATHS=(
    "/opt/codenextremote-data"
    "/var/lib/codenextremote"
    "/opt/rustdesk-data"
    "/var/lib/rustdesk"
    "$HOME/codenextremote-data"
)

# Container names
SERVER_CONTAINER="codenextremote-server"
CONSOLE_CONTAINER="codenextremote-console"
...
# Logging
LOG_FILE="/tmp/codenextremote_docker_$(date +%Y%m%d_%H%M%S).log"
...
auto_detect_docker_paths() {
    ...
    if [ -z "$DATA_DIR" ]; then
        DATA_DIR="/opt/codenextremote-data"
...
configure_docker_paths() {
    ...
    case $choice in
        ...
        2)
            echo ""
            echo -n "Enter data directory path (e.g., /opt/codenextremote-data): "
            read -r new_path
            if [ -n "$new_path" ]; then
                if [ -d "$new_path" ]; then
                    DATA_DIR="$new_path"
        ...
        5)
            DATA_DIR="/opt/codenextremote-data"
            BACKUP_DIR="/opt/codenextremote-backups"
            COMPOSE_FILE="$SCRIPT_DIR/docker-compose.yml"
...
detect_installation() {
    ...
    # Check if images exist (new Go architecture: codenextremote-server + codenextremote-console)
    if docker images | grep -q "codenextremote-server\|codenextremote-console"; then
        IMAGES_BUILT=true
...
print_status() {
    ...
    for image in "codenextremote-server" "codenextremote-console"; do
...
choose_database_type() {
    ...
    case "$db_choice" in
        2)
            ...
            echo ""
            read -p "PostgreSQL password for 'codenextremote' user [codenextremote123]: " pg_pass
            POSTGRESQL_PASS="${pg_pass:-codenextremote123}"
            ;;
        *)
            DB_TYPE="sqlite"
...
EOF
        ...
        if [ -z "$admin_password" ]; then
            ...
            if docker volume inspect "${PROJECT_NAME:-codenextremote}_console_data" >/dev/null 2>&1; then
                print_info "Cleaning old auth database from console_data volume..."
                docker run --rm -v "${PROJECT_NAME:-codenextremote}_console_data:/data" alpine \
        ...
        # Add codenextremote server (Go single binary - signal + relay + API)
        cat >> "$COMPOSE_FILE" << EOF
    server:
...
do_install() {
    ...
    if [ "$INSTALL_STATUS" = "complete" ]; then
        print_warning "codenextremote Docker is already installed!"
...
do_update() {
    ...
    if [ "$INSTALL_STATUS" = "none" ]; then
        print_error "codenextremote Docker is not installed!"
...
do_backup_silent() {
    local backup_name="codenextremote_docker_backup_$(date +%Y%m%d_%H%M%S)"
...
do_diagnostics() {
    ...
    docker stats --no-stream --format "table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}" 2>/dev/null | grep -E "NAME|codenextremote" || echo "No running containers found"
    ...
    local port_issues=0
    local port_defs=(
        "21115:TCP:codenextremote-server:NAT Test"
        "21116:TCP:codenextremote-server:ID Server (TCP)"
        "21116:UDP:codenextremote-server:ID Server (UDP)"
        "21117:TCP:codenextremote-server:Relay Server"
        "5000:TCP:codenextremote-console:Web Console"
        "21121:TCP:codenextremote-console:Client API (WAN)"
    )
...
do_uninstall() {
    ...
    print_warning "This operation will remove codenextremote Docker!"
    echo ""
    ...
    if confirm "Remove Docker images?"; then
        docker rmi codenextremote-server codenextremote-console 2>/dev/null || true
    ...
    print_success "codenextremote Docker has been uninstalled"
    press_enter
}
...
detect_existing_rustdesk() {
    ...
    for pattern in "${container_patterns[@]}"; do
        while IFS= read -r line; do
            [ -z "$line" ] && continue
            # Skip codenextremote containers
            if [[ "$line" == *"codenextremote"* ]]; then
                continue
            fi
    ...
    for base in "${compose_search_paths[@]}"; do
        for fname in "docker-compose.yml" "docker-compose.yaml" "compose.yml" "compose.yaml"; do
            ...
            if [ -f "$candidate" ] && grep -qi "rustdesk\|hbbs\|hbbr" "$candidate" 2>/dev/null; then
                # Skip codenextremote's own compose file
                if grep -qi "codenextremote" "$candidate" 2>/dev/null; then
                    continue
                fi
...
do_migrate() {
    ...
    echo -e "${CYAN}to codenextremote Console with enhanced features and web management.${NC}"
    echo ""
    ...
    echo "  3. Copy data to codenextremote data directory"
    echo "  4. Build and start codenextremote containers"
    ...
    # === Step 3: Prepare codenextremote data directory ===
    print_step "[3/6] Preparing codenextremote data directory..."
    ...
    # === Step 4: Create codenextremote compose file ===
    print_step "[4/6] Creating codenextremote Docker Compose configuration..."
    ...
    print_step "[5/6] Building codenextremote Docker images..."
    
    build_images
    start_containers
    
    # === Step 6: Create admin user ===
    print_step "[6/6] Setting up codenextremote web console..."
...
do_migrate_postgresql() {
    ...
    local arch=$(uname -m)
    case "$arch" in
        x86_64) migrate_tool="./codenextremote-server/tools/migrate/migrate-linux-amd64" ;;
        aarch64|arm64) migrate_tool="./codenextremote-server/tools/migrate/migrate-linux-arm64" ;;
    esac
    ...
    if [ ! -f "$migrate_tool" ]; then
        ...
        echo "  docker exec -it codenextremote-postgres psql -U $POSTGRESQL_USER -d $POSTGRESQL_DB"
        press_enter
        return
    fi
    ...
    cp "$sqlite_db" "/tmp/codenextremote_migrate.sqlite3"
    ...
    docker exec codenextremote-postgres psql -U "$POSTGRESQL_USER" -d "$POSTGRESQL_DB" << 'EOSQL'
...
EOSQL
    ...
    if [ -f /tmp/peers_export.csv ] && [ -s /tmp/peers_export.csv ]; then
        ...
        docker cp /tmp/peers_export.csv codenextremote-postgres:/tmp/peers_export.csv
...
EOSQL
    ...
    if [ -f /tmp/users_export.csv ] && [ -s /tmp/users_export.csv ]; then
        docker cp /tmp/users_export.csv codenextremote-postgres:/tmp/users_export.csv
...
EOSQL
    ...
    local pg_count
    pg_count=$(docker exec codenextremote-postgres psql -U "$POSTGRESQL_USER" -d "$POSTGRESQL_DB" -t -c "SELECT COUNT(*) FROM peers;" | tr -d ' ')
...
do_configure_ssl() {
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            mkdir -p "$ssl_dir"
            cp "$cert_path" "$ssl_dir/codenextremote.crt"
            cp "$key_path" "$ssl_dir/codenextremote.key"
        ...
        5)
            ...
            echo ""
            print_warning "For browsers/clients, you may need to import $ssl_dir/codenextremote.crt as trusted CA"
            ;;
        *)
