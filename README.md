# 🚀 codenextremote v3 — Ultimate Remote Desktop & CDAP Solution

<div align="center">

<img src="codenextremote.png" alt="codenextremote Logo" width="320">
...
- [codenextremote Go Server](#-codenextremote-go-server)
...
**codenextremote** is a complete RustDesk infrastructure solution consisting of two main components:

1. **codenextremote Server** — A clean-room Go implementation that replaces both `hbbs` (signal) and `hbbr` (relay) with a **single binary**. It implements the full RustDesk wire protocol, including UDP/TCP/WebSocket signal, TCP/WebSocket relay, NaCl secure handshake, and a comprehensive HTTP REST API.

2. **codenextremote Console** — A Node.js (Express.js) web management panel with device monitoring, TOTP 2FA, RBAC, address book sync, and RustDesk Client API.

### Why codenextremote?

| Feature | Original RustDesk Server | codenextremote Server |
|---------|------------------------|-------------------|
...
## 🔧 codenextremote Go Server

The Go server (`codenextremote-server/`) is a ~20,000 LOC clean-room implementation of the RustDesk signal and relay protocol. It compiles to a **single static binary** with no external dependencies (pure Go, no CGO required).

### Server Modes

```bash
# Default: run everything (signal + relay + API + admin)
./codenextremote-server -mode all

# Signal only (no relay)
./codenextremote-server -mode signal

# Relay only
./codenextremote-server -mode relay
```
...
```bash
./codenextremote-server -db ./db_v2.sqlite3
```
...
```bash
./codenextremote-server -db "postgres://user:password@localhost:5432/codenextremote?sslmode=disable"
```
...
codenextremote supports TLS on all transport layers with a unique **dual-mode auto-detection** system:

```bash
# Enable TLS on signal ports (21116 TCP + 21115 + 21118 WSS)
./codenextremote-server -tls-signal -tls-cert server.crt -tls-key server.key

# Enable TLS on relay ports (21117 TCP + 21119 WSS)
./codenextremote-server -tls-relay -tls-cert server.crt -tls-key server.key

# Enable TLS on everything
./codenextremote-server -tls-signal -tls-relay -tls-cert server.crt -tls-key server.key

# HTTPS on API
./codenextremote-server -tls-cert server.crt -tls-key server.key -force-https
```
...
```bash
./codenextremote-server -admin-port 9090 -admin-password "secret"
...
chmod +x codenextremote.sh

# Interactive mode (recommended for first install)
sudo ./codenextremote.sh

# Automatic mode (non-interactive)
sudo ./codenextremote.sh --auto

# Skip Go binary SHA256 verification
sudo ./codenextremote.sh --skip-verify

# Custom API port
API_PORT=21120 sudo ./codenextremote.sh --auto
```

The script will:
1. Install Go toolchain if not present
2. Compile `codenextremote-server` from source (single binary)
3. Install Node.js and the web console
4. Create systemd services (`codenextremote-server.service` + `codenextremote-console.service`)
...
The script installs Go, compiles the server, sets up NSSM services (`codenextremoteServer` + `codenextremoteConsole`) or scheduled tasks as fallback.
...
```
/opt/rustdesk/                    # (Linux) or C:\codenextremote\ (Windows)
├── codenextremote-server             # Single Go binary (signal + relay + API)
...
After installing codenextremote server, configure your RustDesk desktop clients to connect to it.
...
| Field | Value | Example |
|-------|-------|---------|
| **ID Server** | Your server IP or domain | `codenextremote.example.com` |
| **Relay Server** | Same as ID Server (or leave empty) | `codenextremote.example.com` |
| **API Server** | `http(s)://<server>:21121` | `http://codenextremote.example.com:21121` |
| **Key** | Contents of `id_ed25519.pub` on the server | (base64 public key string) |

> **Tip:** The public key can be found in the Web Console under **Dashboard → Server Keys**, or by reading the file `/opt/rustdesk/id_ed25519.pub` (Linux) / `C:\codenextremote\id_ed25519.pub` (Windows) on the server.
...
```json
{
  "host": "codenextremote.example.com",
  "relay": "codenextremote.example.com",
  "api": "http://codenextremote.example.com:21121",
...
```bash
rustdesk --config '{"host":"codenextremote.example.com","key":"<pubkey>"}'
```

### Desktop Client Login

RustDesk desktop clients can **log in** to the codenextremote server using their user account. This enables address book synchronization, device grouping, and audit trail per user.
...
Connecting the RustDesk desktop client to a codenextremote server with the API Server field configured **automatically activates Pro-level features** — no license key required. These features are built into the standard RustDesk client but remain dormant until a compatible API server is detected.
...
| Field | Value |
|-------|-------|
| **ID Server** | `codenextremote.example.com` |
| **Relay Server** | `codenextremote.example.com` (or leave empty to auto-detect) |
| **API Server** | `http://codenextremote.example.com:21121` |
| **Key** | Contents of `id_ed25519.pub` from the server |

4. Click the **account icon** (bottom-left) and **log in** with your codenextremote credentials
...
Or via command line:
```bash
rustdesk --config '{"host":"codenextremote.example.com","relay":"codenextremote.example.com","api":"http://codenextremote.example.com:21121","key":"<pubkey>"}'
```
...
codenextremote supports TLS on all layers: Go server transport (signal + relay), Go server HTTPS API, and the Node.js web console.
...
openssl req -x509 -nodes -days 1095 -newkey rsa:2048 \
  -keyout /opt/rustdesk/ssl/codenextremote.key \
  -out /opt/rustdesk/ssl/codenextremote.crt \
  -subj "/CN=$(hostname -f)/O=codenextremote/C=US" \
  -addext "subjectAltName=DNS:$(hostname -f),DNS:localhost,IP:$(curl -s ifconfig.me),IP:127.0.0.1"

# Secure the private key
chmod 600 /opt/rustdesk/ssl/codenextremote.key
```

> **Windows (PowerShell)**:
> ```powershell
> New-Item -ItemType Directory -Path "C:\codenextremote\ssl" -Force
> openssl req -x509 -nodes -days 1095 -newkey rsa:2048 `
>   -keyout "C:\codenextremote\ssl\codenextremote.key" `
>   -out "C:\codenextremote\ssl\codenextremote.crt" `
>   -subj "/CN=localhost/O=codenextremote"
> ```
...
./codenextremote-server -tls-signal -tls-cert /opt/rustdesk/ssl/codenextremote.crt -tls-key /opt/rustdesk/ssl/codenextremote.key

# TLS on relay ports (21117 TCP + 21119 WSS)
./codenextremote-server -tls-relay -tls-cert /opt/rustdesk/ssl/codenextremote.crt -tls-key /opt/rustdesk/ssl/codenextremote.key

# TLS everywhere + force HTTPS on API
./codenextremote-server -tls-signal -tls-relay -force-https \
  -tls-cert /opt/rustdesk/ssl/codenextremote.crt \
  -tls-key /opt/rustdesk/ssl/codenextremote.key
```

For systemd, add the flags to `ExecStart` in `/etc/systemd/system/codenextremote-server.service`:
```ini
ExecStart=/opt/rustdesk/codenextremote-server -mode all ... \
  -tls-signal -tls-relay \
  -tls-cert /opt/rustdesk/ssl/codenextremote.crt \
  -tls-key /opt/rustdesk/ssl/codenextremote.key
```

> **Dual-mode**: codenextremote auto-detects plain vs TLS on the **same port** (first-byte `0x16` detection). Existing non-TLS clients continue to work without changes.
...
```bash
HTTPS_ENABLED=true
SSL_CERT_PATH=/opt/rustdesk/ssl/codenextremote.crt
SSL_KEY_PATH=/opt/rustdesk/ssl/codenextremote.key
...
sudo certbot certonly --standalone -d codenextremote.example.com

# Certificate files:
# /etc/letsencrypt/live/codenextremote.example.com/fullchain.pem
# /etc/letsencrypt/live/codenextremote.example.com/privkey.pem
```
...
The ALL-IN-ONE scripts (`codenextremote.sh` / `codenextremote.ps1`) **automatically generate a self-signed certificate** during every fresh installation:

1. **Certificate location**: `<RUSTDESK_PATH>/ssl/codenextremote.crt` + `codenextremote.key`
...
### From Original RustDesk (Rust) to codenextremote (Go)

The migration tool (`codenextremote-server/tools/migrate/`) supports multiple migration paths:

**Linux/macOS:**
```bash
# Compile migration tool
cd codenextremote-server/tools/migrate
go build -o migrate .

# Mode 1: Rust hbbs → codenextremote Go (preserves peer table → peers)
./migrate -mode rust2go -src /opt/rustdesk/db_v2.sqlite3 -dst /opt/codenextremote/db_v2.sqlite3

# Mode 2: SQLite → PostgreSQL
./migrate -mode sqlite2pg -src /opt/codenextremote/db_v2.sqlite3   -dst "postgres://user:pass@localhost:5432/codenextremote"

# Mode 3: PostgreSQL → SQLite (reverse)
./migrate -mode pg2sqlite -src "postgres://..." -dst ./backup.sqlite3

# Mode 4: Node.js console → Go server
./migrate -mode nodejs2go -src /opt/codenextremote/web-console/codenextremote.db   -dst /opt/codenextremote/db_v2.sqlite3

# Mode 5: Backup
./migrate -mode backup -src /opt/codenextremote/db_v2.sqlite3
```

**Windows (PowerShell):**
```powershell
# Compile migration tool (requires Go installed)
cd codenextremote-server\tools\migrate
go build -o migrate.exe .

# Usage (same modes as Linux)
.\migrate.exe -mode rust2go -src C:\codenextremote\db_v2.sqlite3 -dst C:\codenextremote\db_v2_new.sqlite3
.\migrate.exe -mode sqlite2pg -src C:\codenextremote\db_v2.sqlite3 -dst "postgres://user:pass@localhost:5432/codenextremote"
```

> **Note:** Windows users need [Go](https://go.dev/dl/) installed to compile the migration tool. Pre-built binaries are available in [GitHub Releases](https://github.com/shamstabraiz/codenextremote/releases) (when available).

The migration tool auto-detects the source schema (original RustDesk `peer` table vs codenextremote `peers` table) and maps columns accordingly. Ed25519 keys, UUIDs, ID history, bans, and tags are fully preserved.

### Using ALL-IN-ONE Scripts

Both `codenextremote.sh` and `codenextremote.ps1` include built-in migration options:
...
The wizard auto-detects existing containers, creates a backup, and migrates data to codenextremote.
...
```
# Counters (monotonic)
codenextremote_registrations_total
codenextremote_expired_total
codenextremote_relay_sessions_total
codenextremote_relay_bytes_total
codenextremote_bandwidth_throttle_hits_total
codenextremote_audit_events_total

# Gauges (current values)
codenextremote_uptime_seconds
codenextremote_peers_total
codenextremote_peers_online
codenextremote_peers_degraded
codenextremote_peers_critical
codenextremote_peers_offline
codenextremote_peers_banned
codenextremote_peers_udp
codenextremote_peers_tcp
codenextremote_peers_ws
codenextremote_relay_active_sessions
codenextremote_blocklist_entries
codenextremote_event_subscribers
codenextremote_goroutines
codenextremote_memory_alloc_bytes
codenextremote_memory_sys_bytes
```
...
./codenextremote-server -audit-log /var/log/codenextremote/audit.jsonl
```
...
| **All devices offline** | Database missing `last_online` column | Run repair: `sudo ./codenextremote.sh` → option 3 |
...
sudo ./codenextremote.sh  # Select option 8

# Check service status
sudo systemctl status codenextremote-server
sudo systemctl status codenextremote-console

# Check logs
sudo journalctl -u codenextremote-server -n 100 --no-pager
sudo journalctl -u codenextremote-console -n 100 --no-pager
...
Get-Service codenextremoteServer
Get-Service codenextremoteConsole
```
...
RustDesk clients support end-to-end encryption for remote desktop sessions. codenextremote Server fully supports this — both P2P (hole-punch) and relay-mode connections establish an encrypted E2E channel with NaCl key exchange.
...
codenextremote includes a built-in chat system between operators and end-user devices with full end-to-end encryption.
...
| Component | Technology |
|-----------|-----------|
| **Linux services** | systemd (`codenextremote-server.service` + `codenextremote-console.service`) |
| **Windows services** | NSSM (`codenextremoteServer` + `codenextremoteConsole`) |
| **Docker** | Docker Compose with local image builds |
| **Installation** | Bash (`codenextremote.sh`) + PowerShell (`codenextremote.ps1`) ALL-IN-ONE |
...
```bash
cd codenextremote-server

# Linux (amd64)
CGO_ENABLED=0 go build -ldflags="-s -w" -o codenextremote-server .

# Linux (arm64)
CGO_ENABLED=0 GOARCH=arm64 go build -ldflags="-s -w" -o codenextremote-server-arm64 .

# Windows
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -o codenextremote-server.exe .
```
...
codenextremote Console supports 25+ languages through JSON-based translations with auto-discovery.
...
### codenextremote MGMT Client (Operator/Admin)
...
**Build**: `cd codenextremote-mgmt && pnpm install && pnpm tauri build`

### codenextremote Agent Client (Endpoint Device)
...
**Build**: `cd codenextremote-agent-client && pnpm install && pnpm tauri build`
...
codenextremote includes a browser-based remote desktop client accessible from the Web Console.
...
├── codenextremote-server/           # Go server (~20K LOC)
...
├── codenextremote-mgmt/             # MGMT Desktop Client (Tauri v2 + SolidJS)
├── codenextremote-agent-client/     # Agent Client (Tauri v2 + SolidJS)
├── codenextremote-agent/            # Native CDAP Agent (Go)
├── sdks/                        # CDAP Bridge SDKs (Python + Node.js)
├── bridges/                     # Reference CDAP Bridges (Modbus, SNMP, REST)
├── codenextremote.sh                # Linux ALL-IN-ONE installer (v2.4.0)
├── codenextremote.ps1               # Windows ALL-IN-ONE installer (v2.4.0)
├── codenextremote-docker.sh         # Docker installer (v2.4.0)
...
1. Run diagnostics: `sudo ./codenextremote.sh` → option 8
2. Collect logs: `journalctl -u codenextremote-server -n 100`
...
codenextremote Server (`codenextremote-server/`) is a **clean-room implementation** of the RustDesk rendezvous and relay protocol. Like how any HTTP server implements the HTTP protocol without being "derived from" Apache or Nginx, codenextremote implements published protocol specifications for compatibility with RustDesk clients — but contains **no RustDesk source code**.
...
codenextremote Console (`web-nodejs/`) is an entirely original Node.js/Express application.
...
"RustDesk" is a trademark of the RustDesk Team. codenextremote is an independent project that implements the RustDesk protocol for client compatibility. Use of the name "RustDesk" in this project is purely descriptive (indicating protocol compatibility) and does not imply affiliation with or endorsement by the RustDesk Team.
...
[⬆ Back to Top](#-codenextremote--rustdesk-compatible-server--web-console)
