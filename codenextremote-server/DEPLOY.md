# codenextremote Go Server — Deployment Guide

## Overview

codenextremote Go Server is a **single binary** that replaces the Rust `hbbs` (signal) + `hbbr` (relay) servers. It is fully compatible with original RustDesk clients and provides additional features:
...
- Existing RustDesk/codenextremote installation (optional, for migration)
...
scp codenextremote-server-linux-amd64 user@server:/tmp/
...
# Check logs
journalctl -u codenextremote -f

# Get initial admin credentials (printed at first start)
journalctl -u codenextremote | grep "INITIAL ADMIN"
```
...
cp /opt/rustdesk/db_v2.sqlite3 /opt/codenextremote/backups/db_v2.sqlite3.${TIMESTAMP}
cp /opt/rustdesk/db_v2.sqlite3-wal /opt/codenextremote/backups/db_v2.sqlite3-wal.${TIMESTAMP} 2>/dev/null
cp /opt/rustdesk/db_v2.sqlite3-shm /opt/codenextremote/backups/db_v2.sqlite3-shm.${TIMESTAMP} 2>/dev/null

# Backup key files
cp /opt/rustdesk/id_ed25519 /opt/codenextremote/backups/id_ed25519.${TIMESTAMP}
cp /opt/rustdesk/id_ed25519.pub /opt/codenextremote/backups/id_ed25519.pub.${TIMESTAMP}
```
...
```bash
# Run migration tool
./migrate-linux-amd64 -src /opt/rustdesk/db_v2.sqlite3 -dst /opt/codenextremote/data/db_v2.sqlite3
```
...
```bash
sudo mkdir -p /opt/codenextremote/data /var/log/codenextremote
sudo cp codenextremote-server-linux-amd64 /opt/codenextremote/codenextremote-server
sudo chmod +x /opt/codenextremote/codenextremote-server

# Copy existing key file (important for client compatibility!)
sudo cp /opt/rustdesk/id_ed25519 /opt/codenextremote/data/
sudo cp /opt/rustdesk/id_ed25519.pub /opt/codenextremote/data/
```

### Step 5: Create systemd service

```bash
sudo cat > /etc/systemd/system/codenextremote.service << 'EOF'
[Unit]
Description=codenextremote Server (Signal + Relay + API)
After=network.target

[Service]
Type=simple
User=root
WorkingDirectory=/opt/codenextremote/data
ExecStart=/opt/codenextremote/codenextremote-server \
    -mode all \
    -db /opt/codenextremote/data/db_v2.sqlite3 \
    -key-file /opt/codenextremote/data/id_ed25519 \
    -port 21116 \
    -relay-port 21117 \
    -api-port 21114 \
    -admin-port 21000 \
    -log-format json \
    -audit-log /var/log/codenextremote/audit.jsonl
Restart=always
RestartSec=5
LimitNOFILE=65535
...
sudo systemctl enable codenextremote
...
sudo systemctl disable codenextremote

# Restore backup
cp /opt/codenextremote/backups/backup_YYYYMMDD_HHMMSS/db_v2.sqlite3 /opt/rustdesk/
cp /opt/codenextremote/backups/backup_YYYYMMDD_HHMMSS/id_ed25519 /opt/rustdesk/
...
```bash
# Check logs
journalctl -u codenextremote -n 100 --no-pager

# Common issues:
# - Port already in use → check if old hbbs/hbbr is still running
# - Permission denied → ensure /opt/codenextremote/data is writable
...
2. **Check signal logs**: `journalctl -u codenextremote | grep RegisterPeer`
