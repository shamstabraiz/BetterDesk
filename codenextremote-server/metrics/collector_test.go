...
func TestWritePrometheus(t *testing.T) {
	...
	if !strings.Contains(output, "# HELP codenextremote_peers_total") {
		t.Error("missing HELP for peers_total")
	}
	if !strings.Contains(output, "# TYPE codenextremote_peers_total gauge") {
		t.Error("missing TYPE for peers_total")
	}
	if !strings.Contains(output, "codenextremote_peers_total 100") {
		t.Error("missing peers_total value")
	}
	if !strings.Contains(output, "codenextremote_peers_online 42") {
		t.Error("missing peers_online value")
	}
	if !strings.Contains(output, "codenextremote_registrations_total 500") {
		t.Error("missing registrations_total value")
	}
	if !strings.Contains(output, "codenextremote_relay_active_sessions 5") {
		t.Error("missing relay_active_sessions value")
	}

	// Should contain uptime
	if !strings.Contains(output, "codenextremote_uptime_seconds") {
		t.Error("missing uptime_seconds")
	}

	// Should contain goroutines (runtime metric)
	if !strings.Contains(output, "codenextremote_goroutines") {
