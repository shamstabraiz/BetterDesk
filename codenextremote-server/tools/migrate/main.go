// migrate — codenextremote database migration tool.
//
// Supports multiple migration modes:
//
//	rust2go   — Rust hbbs-patch-v2 SQLite → codenextremote Go schema (SQLite or PG)
//	sqlite2pg — codenextremote Go SQLite → PostgreSQL
//	pg2sqlite — PostgreSQL → codenextremote Go SQLite
//	nodejs2go — Node.js console SQLite → codenextremote Go schema (SQLite or PG)
...
//	./migrate -mode nodejs2go -src /opt/codenextremote/data/db_v2.sqlite3 -node-auth /opt/codenextremote/data/auth.sqlite3
...
func printUsage() {
	fmt.Println("codenextremote Database Migration Tool")
	fmt.Println()
	fmt.Println("Usage: migrate -mode <mode> -src <source> [-dst <dest>] [options]")
	fmt.Println()
	fmt.Println("Modes:")
	fmt.Println("  rust2go   - Migrate Rust hbbs-patch-v2 → codenextremote Go schema")
	fmt.Println("  sqlite2pg - Migrate codenextremote Go SQLite → PostgreSQL")
	fmt.Println("  pg2sqlite - Migrate PostgreSQL → codenextremote Go SQLite")
	fmt.Println("  nodejs2go - Migrate Node.js console → codenextremote Go schema")
	...
	fmt.Println("  ./migrate -mode sqlite2pg -src ./db_v2.sqlite3 -dst postgres://user:pass@host/codenextremote")
	fmt.Println("  ./migrate -mode pg2sqlite -src postgres://user:pass@host/codenextremote -dst ./db_v2.sqlite3")
...
func runRust2Go(srcPath, dstPath string) {
	...
	if dstPath == "" {
		dstPath = filepath.Join(filepath.Dir(srcPath), "codenextremote.sqlite3")
	}
...
// writeRustPeersSQLite writes Rust peers to a codenextremote Go SQLite database.
...
// writeRustPeersPG writes Rust peers to a codenextremote Go PostgreSQL database.
...
func runNodeJS2Go(srcPeerPath, srcAuthPath, dstPath string) {
	...
	if dstPath == "" {
		dstPath = filepath.Join(filepath.Dir(srcPeerPath), "codenextremote.sqlite3")
	}
...
// pgSchemaStatements returns PostgreSQL DDL for all codenextremote Go tables.
...
// sqliteSchemaStatements returns SQLite DDL for all codenextremote Go tables.
