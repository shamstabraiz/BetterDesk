#!/usr/bin/env bash
# ============================================================================
# codenextremote Go Server — Production Deployment Script
# Version: 1.0.0
# Replaces: hbbs + hbbr (Rust) with a single codenextremote-server binary (Go)
#
# Usage:
...
# Default paths (matching codenextremote.sh conventions)
INSTALL_DIR="/opt/codenextremote"
OLD_INSTALL_DIR="/opt/rustdesk"
DATA_DIR="${INSTALL_DIR}/data"
BACKUP_DIR="${INSTALL_DIR}/backups"
LOG_DIR="/var/log/codenextremote"
SERVICE_NAME="codenextremote"

# Binary names
GO_BINARY="codenextremote-server-linux-amd64"
...
detect_existing_installation() {
    ...
    for dir in /opt/rustdesk /opt/codenextremote /opt/hbbs; do
        if [[ -d "$dir" ]]; then
...
create_backup() {
    ...
    for svc in rustdesksignal rustdeskrelay codenextremote hbbs hbbr; do
    ...
    if [[ -n "$EXISTING_DIR" ]]; then
        for bin in hbbs hbbr codenextremote-server; do
...
install_binary() {
    log_step "Installing codenextremote Go server..."

    mkdir -p "$INSTALL_DIR" "$DATA_DIR" "$LOG_DIR" "$BACKUP_DIR"

    # Copy binary
    cp "${SCRIPT_DIR}/${GO_BINARY}" "${INSTALL_DIR}/codenextremote-server"
    chmod +x "${INSTALL_DIR}/codenextremote-server"
    ...
    log_info "Binary installed: ${INSTALL_DIR}/codenextremote-server"
}

stop_old_services() {
    log_step "Stopping old services..."

    for svc in rustdesksignal rustdeskrelay hbbs hbbr codenextremote; do
...
ReadWritePaths=/opt/codenextremote/data /var/log/codenextremote
...
enable_and_start() {
    log_step "Enabling and starting codenextremote server..."
    ...
    if systemctl is-active --quiet "${SERVICE_NAME}"; then
        log_info "codenextremote server is running!"
    else
...
configure_firewall() {
    ...
    if command -v ufw &>/dev/null; then
        ufw allow 21114/tcp comment "codenextremote API"
        ufw allow 21115/tcp comment "codenextremote NAT Test"
        ufw allow 21116/tcp comment "codenextremote Signal TCP"
        ufw allow 21116/udp comment "codenextremote Signal UDP"
        ufw allow 21117/tcp comment "codenextremote Relay"
        ufw allow 21118/tcp comment "codenextremote WS Signal"
        ufw allow 21119/tcp comment "codenextremote WS Relay"
...
print_banner() {
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  codenextremote Go Server — Deployment v${VERSION}"
...
main() {
    ...
    if ! $AUTO; then
        ...
        echo "  4. Install the codenextremote Go server"
