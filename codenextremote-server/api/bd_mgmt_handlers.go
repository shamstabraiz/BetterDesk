...
// codenextremote Desktop Client Management WebSocket
//
// Endpoint: GET /ws/bd-mgmt/{device_id}
//
// This WebSocket channel replaces CDAP for desktop clients. It provides:
//   - Device identification (the server marks the peer as codenextremote_desktop)
...
// bdMgmtSession tracks a single connected codenextremote desktop client.
...
// bdMgmtHub manages all active codenextremote desktop management sessions.
...
// IsConnected checks if a codenextremote desktop client is connected.
...
func (s *Server) handleBdMgmt(w http.ResponseWriter, r *http.Request) {
	...
	// Mark device as codenextremote_desktop in the database
	_ = s.db.UpdatePeerFields(deviceID, map[string]string{
		"device_type": "codenextremote_desktop",
	})
...
// handleBdMgmtConnected returns all connected codenextremote desktop clients.
