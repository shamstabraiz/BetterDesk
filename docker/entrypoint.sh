#!/bin/sh
# codenextremote — All-in-One Container Entrypoint
# Runs Go server + Node.js console via supervisord
set -e

echo "========================================"
echo "  codenextremote All-in-One Container"
echo "  Version: 2.4.0"
echo "========================================"
echo ""
echo "Components:"
...
mkdir -p /opt/rustdesk /app/data /var/log/codenextremote 2>/dev/null || true
chown -R codenextremote:codenextremote /opt/rustdesk /app/data /var/log/codenextremote 2>/dev/null || true
# Verify write access — SQLite WAL mode requires writable directory (Issue #78)
if ! su -s /bin/sh codenextremote -c 'touch /opt/rustdesk/.write_test' 2>/dev/null; then
    echo ""
    echo "ERROR: /opt/rustdesk is NOT writable by the codenextremote user (UID 10001)."
...
if [ -f /opt/rustdesk/id_ed25519 ]; then
    ...
    chown codenextremote:codenextremote /opt/rustdesk/id_ed25519
fi
...
if [ -z "${API_KEY:-}" ] && [ ! -f "$API_KEY_FILE" ]; then
    ...
    chown codenextremote:codenextremote "$API_KEY_FILE" 2>/dev/null || true
...
exec /usr/bin/supervisord -c /etc/supervisor/conf.d/codenextremote.conf
