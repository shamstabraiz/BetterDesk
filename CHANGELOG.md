...
### Added
- **RustDesk PRO group endpoint stubs** — `codenextremote-server/api/server.go` now exposes `GET /api/group`, `GET|POST /api/group/get`, and `GET /api/peers/list` returning the `{total, data, msg}` envelope expected by RustDesk Flutter clients. Without these endpoints the Flutter UI aborted device-list loading and never fell back to address-book mode. Idea credit: [progloto](https://github.com/progloto) ([PR #81](https://github.com/shamstabraiz/codenextremote/pull/81)).
- **Catch-all 404 logging** — Both `codenextremote-server/api/server.go` (Go API) and `web-nodejs/server.js` (RustDesk-compatible API + main panel) now log unmatched routes with method, path, client IP, and User-Agent. Makes missing client-compatibility endpoints easy to spot during deployments. Diagnostics suggestion credit: [progloto](https://github.com/progloto) ([PR #81](https://github.com/shamstabraiz/codenextremote/pull/81)).
- **Defensive list parsing in panel** — `web-nodejs/services/codenextremoteApi.js` and `web-nodejs/public/js/devices.js` already accepted both `[…]` and `{ peers: […] }` shapes; this stays unchanged so admin UI works regardless of which endpoint envelope a future RustDesk version returns.
...
- **mDNS/DNS-SD Discovery** — Auto-discover codenextremote servers on LAN (`_codenextremote._tcp`)
...
- **Native codenextremote Agent** — Go binary for system management, 14 flags, 9 widgets
...
- **ALL-IN-ONE Scripts** — `codenextremote.sh` + `codenextremote.ps1` + `codenextremote-docker.sh`
- **Automatic Mode** — `--auto` flag for non-interactive installation
- **SHA256 Verification** — Automatic checksum verification of binaries

---

[3.0.0-alpha]: https://github.com/shamstabraiz/codenextremote/compare/v2.4.0...HEAD
[2.4.0]: https://github.com/shamstabraiz/codenextremote/compare/v2.3.0...v2.4.0
[2.3.0]: https://github.com/shamstabraiz/codenextremote/compare/v2.2.0...v2.3.0
[2.2.0]: https://github.com/shamstabraiz/codenextremote/compare/v2.1.0...v2.2.0
[2.1.0]: https://github.com/shamstabraiz/codenextremote/releases/tag/v2.1.0
