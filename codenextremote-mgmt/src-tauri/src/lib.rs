//! codenextremote MGMT Client — operator and admin remote management console.
//!
//! Architecture:
...
pub fn run() {
    ...
    // Initialize dual logging: stderr + file in %APPDATA%/codenextremote/codenextremote/logs/
    logging::init(is_console);

    info!("codenextremote MGMT v{} (pid={}) — boot", env!("CARGO_PKG_VERSION"), std::process::id());
    if is_console {
    ...
    tauri::Builder::default()
                ...
                match identity::get_or_create_device_id() {
                    Ok(device_id) => {
                        ...
                        if settings.native_protocol {
                            // codenextremote native — HTTP registration + WebSocket relay
                            info!(
        ...
        .expect("error while running codenextremote");

    // App loop exited — signal all services to stop.
    shutdown_token.cancel();
    info!("codenextremote shutdown complete");
}
