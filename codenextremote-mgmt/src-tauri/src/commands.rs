...
//!
//! Commands transparently dispatch between codenextremote native protocol (HTTP +
...
/// (preserves backwards compatibility with existing codenextremote deployments).
...
fn warn_self_signed_once() {
    ...
    if !WARNED.swap(true, Ordering::SeqCst) {
        eprintln!(
            "[codenextremote] WARNING: TLS certificate validation is DISABLED for \
...
pub struct AppState {
    ...
    /// codenextremote native HTTP registration service.
    pub bd_registration: Mutex<Option<BdRegistrationService>>,
    /// codenextremote native WebSocket relay connection.
...
pub struct UnifiedRegistrationStatus {
    ...
    /// true when using codenextremote native protocol.
...
/// Browse for codenextremote servers via mDNS/DNS-SD.
/// Returns servers advertising `_codenextremote._tcp` on the local network.
#[tauri::command]
...
/// Test connection to a codenextremote server without saving config.
///
...
/// Auto-connect to a codenextremote server using only the IP/hostname.
///
...
pub async fn auto_connect_server(
    ...
    let device_id = crate::identity::get_or_create_device_id().unwrap_or_default();
    {
        ...
        if settings.native_protocol {
            // codenextremote native: HTTP enrollment + CDAP + management
...
/// Login to an organization account on the codenextremote server.
...
/// Proxy an HTTP request to the codenextremote web panel through Rust.
///
