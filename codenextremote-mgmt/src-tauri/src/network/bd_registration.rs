//! codenextremote native registration — Go server-centric enrollment.
//!
...
impl BdRegistrationService {
    /// Start the codenextremote native registration service.
    ...
    pub fn stop(&self) {
        ...
        info!("codenextremote registration service stop requested");
    }
}
...
async fn do_enroll(
    ...
    let body = EnrollmentRequest {
        ...
        device_type: "codenextremote".to_string(),
...
fn get_uuid() -> Result<String> {
    let config_dir = directories::ProjectDirs::from("com", "codenextremote", "codenextremote")
...
fn load_or_create_signing_key() -> Result<SigningKey> {
    ...
    info!("Generated Ed25519 keypair for codenextremote management authentication");
    Ok(signing_key)
}
