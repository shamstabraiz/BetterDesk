/// codenextremote Agent — Script Runner
///
...
impl ScriptRunner {
    ...
    fn run_script(payload: &str) -> Result<(String, String, i32)> {
        ...
        let script_path = temp_dir.join(format!("codenextremote_script_{}{}", std::process::id(), ext));
    ...
    fn reboot() -> Result<(String, String, i32)> {
        if cfg!(target_os = "windows") {
            Self::run_shell("shutdown /r /t 30 /c \"codenextremote remote reboot\"")
        } else {
            Self::run_shell("shutdown -r +1 'codenextremote remote reboot'")
        }
    }
}
