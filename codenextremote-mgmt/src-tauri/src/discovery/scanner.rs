//! UDP broadcast scanner for codenextremote servers on the local network.
//!
...
/// A discovered codenextremote server with connection details.
...
async fn send_probe_and_collect() -> Result<Vec<(SocketAddr, ServerInfo)>> {
    ...
    let probe = DiscoverProbe {
        msg_type: "codenextremote-discover".into(),
        version: 1,
    };
