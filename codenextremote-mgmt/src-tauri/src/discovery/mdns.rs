//! mDNS/DNS-SD discovery for codenextremote servers on the local network.
//!
//! Discovers servers advertising `_codenextremote._tcp.local.` via mDNS.
...
const SERVICE_TYPE: &str = "_codenextremote._tcp.local.";
const BROWSE_TIMEOUT: Duration = Duration::from_secs(15);

/// A codenextremote server discovered via mDNS.
...
/// Browse for codenextremote servers via mDNS for `timeout` duration.
        ...
        match receiver.recv_timeout(std::time::Duration::from_millis(500)) {
            Ok(ServiceEvent::ServiceResolved(info)) => {
                ...
                let server = MdnsServer {
                    name: info.get_fullname().split('.').next().unwrap_or("codenextremote").to_string(),
                    host: host.clone(),
                    port: info.get_port(),
                    addresses,
                    version,
