...
impl Default for Branding {
    fn default() -> Self {
        Self {
            company_name: "codenextremote".into(),
...
pub fn setup_tray<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    TrayIconBuilder::with_id("codenextremote-tray")
        .tooltip("codenextremote — right-click for menu")
        .icon(icon)
        .menu(&menu)
...
fn build_tray_menu<R: Runtime>(
    ...
    // Header: company name — codenextremote (disabled, text-only branding)
    let header_text = if branding.company_name.is_empty() || branding.company_name == "codenextremote" {
        "codenextremote".to_string()
    } else {
        format!("{} — codenextremote", branding.company_name)
    };
...
fn rebuild_tray_menu<R: Runtime>(app: &AppHandle<R>, branding: &Branding) -> tauri::Result<()> {
    let menu = build_tray_menu(app, branding)?;
    if let Some(tray) = app.tray_by_id("codenextremote-tray") {
...
fn branding_cache_path() -> Option<std::path::PathBuf> {
    directories::ProjectDirs::from("com", "codenextremote", "codenextremote")
