/// codenextremote Agent — File Sender
///
...
mod tests {
    ...
    fn test_header_generation() {
        ...
        let path = dir.join("codenextremote_test_sender.txt");
        let mut f = std::fs::File::create(&path).unwrap();
        f.write_all(b"Hello, codenextremote!").unwrap();
        drop(f);

        let sender = FileSender::new(&path).unwrap();
        let header = sender.header().unwrap();
        assert_eq!(header.size, 18);
        assert_eq!(header.filename, "codenextremote_test_sender.txt");
